...
code.** Repo's FIRST Coq→OCaml extraction. `proofs/CompileGuaranteeExtract.v` extracts
`project_wf_dec` (+deps) to `compile_guarantee_extracted.ml` (committed; reproducibly
regenerated byte-identically by `scripts/tools/regen_compile_guarantee_extract.sh`;
`nat`→`int` via `ExtrOcamlNatInt`). `Compile_evidence`'s hand-written mirror premise-
checkers are ELIMINATED — it delegates the whole premise decision to the extracted
module (`Ext.*`, 50 sites), so `--compile-check` MODEL-READY reflects the PROVEN-EXTRACTED
decision (`project_wf_dec_sound` governs executed code). Capstone + bridge theorems still
Qed / Print Assumptions Closed / 0-admit; 13 tests incl. extracted==Coq-Example. Honest
residual: the proven core sits between two TRUSTED ends — bytes→body_token extraction
(`Parser_l2`+`Ast_semantic_state`) and the runtime→extracted-type glue; `Parser_l2`
byte→AST formalization is the irreducible research-scale remainder.
