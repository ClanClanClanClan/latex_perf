...
code.** The repo's FIRST Coq→OCaml extraction. `proofs/CompileGuaranteeExtract.v`
extracts `project_wf_dec` (+ all dependencies) to `compile_guarantee_extracted.ml`
(committed; reproducibly regenerated byte-identically by
`scripts/tools/regen_compile_guarantee_extract.sh`; `nat`→OCaml `int` via
`ExtrOcamlNatInt` for the 30-bit hash label-ids). `Compile_evidence`'s hand-written
mirror premise-checkers are **eliminated** — it now delegates the entire premise
decision to the extracted module (`Ext.*`, 50 call sites), so `--compile-check`
MODEL-READY reflects the **proven-extracted** decision (`project_wf_dec_sound` governs
executed code), not a tested-equivalent reimplementation. Capstone + bridge theorems
still Qed / Print Assumptions Closed / 0-admit; 13 tests (incl. extracted==Coq-Example
agreement). **Honest residual after this:** the proven core sits between two TRUSTED
ends — (i) bytes→body_token extraction (`Parser_l2` + `Ast_semantic_state` + feature
scan), (ii) the small runtime→extracted-type conversion glue. Both hand-written +
tested, not machine-verified; `Parser_l2` byte→AST formalization is the irreducible
research-scale remainder. Theorem count 1,484.
