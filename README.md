...
- **Proofs**: 179 Coq files, 1,592 theorems/lemmas. 0 admits, 0 axioms. Per-rule proof classes: 637 formal_faithful + 20 formal_conservative + 3 formal_conditional = 660, i.e. every catalogued rule (the figure previously published here as "643 per-rule soundness" was the non-reserved rule headcount, not a proof count, and its three parts never summed to it — see OPEN-014/OPEN-051). ML: `v2_span_extractor_sound` QED.
- **Validators**: 643 rule IDs / 660 spec. 167 fix-producing rules (Bucket A) + 124 Bucket-C candidate rules. 19 L3 file-based + 12 expl3 rules.
...
- **Proof strategy**: 0 admits, 0 axioms; 179 Coq files, 1,592 theorems/lemmas.
