...
let update_stats stats diff =
  if diff = 0 then (
    stats.zero_diffs <- stats.zero_diffs + 1
  ) else (
    ...
    let current = try Hashtbl.find stats.deviation_histogram diff with Not_found -> 0 in
...
let print_stats stats duration =
  ...
  printf "Perfect Matches (diff=0): %d (%.2f%%)\n" 
    stats.zero_diffs 
    (100.0 *. float_of_int stats.zero_diffs /. float_of_int stats.total_iterations);
  ...
  if stats.deviations_found = 0 then (
    ...
    printf "Deviation Rate: %.6f%% (%d/%d)\n"
      (100.0 *. float_of_int stats.deviations_found /. float_of_int stats.total_iterations)
    ...
    let hist_list = Hashtbl.fold (fun k v acc -> (k, v) :: acc) stats.deviation_histogram [] in
    let sorted = List.sort (fun (_, v1) (_, v2) -> compare v2 v1) hist_list in
    List.iteri (fun i (diff, count) ->
      if i < 10 then
        printf "  diff=%d: %d occurrences\n" diff count
    ) sorted
  )
...
let run_spec_validation_test file_path iterations =
  ...
  for i = 1 to iterations do
    if i mod progress_interval = 0 then (
      printf "\rProgress: %d/%d (%.1f%%)" i iterations (100.0 *. float_of_int i /. float_of_int iterations);
      flush stdout
    );
    
    try
      let (simd_buf, scalar_buf, diff) = Simd_val.benchmark_simd_vs_scalar input_bytes 10000 in
...
let () =
  ...
  try
    run_spec_validation_test test_file iterations
  with
  | Failure msg -> printf "FATAL ERROR: %s\n" msg; exit 2
