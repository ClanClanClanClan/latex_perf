...
let run_single_tokenization_test file_path =
  ...
  let cmd = sprintf "OPAMSWITCH=l0-testing opam exec -- ./_build/default/latex-parse/bench/ab_microbench.exe %s 1" file_path in
  try
...
let extract_result_info output = 
  ...
  if String.contains output 'N' && String.contains output '=' then
    `Success
...
let run_comprehensive_validation file_path total_iterations =
  ...
  for i = 1 to total_iterations do
    if i mod (max 1 (total_iterations / 50)) = 0 then (
      printf "\rProgress: %d/%d (%.1f%%) | Success: %d, Fail: %d, Crash: %d" 
  ...
  printf "Successful Tokenizations: %d (%.2f%%)\n" 
    ...
    (100.0 *. float_of_int stats.successful_runs /. float_of_int total_iterations);
  ...
  let total_issues = stats.failed_runs + stats.crashes + stats.inconsistent_results in
