...
type broker = {
  ...
  mutable stamp_hooks : (int64 -> unit) * (int64 -> unit);
      (* hedge_fire, first_reply *)
}

let create_broker primary_path secondary_path =
  {
    primary =
      { socket_path = primary_path; available = true; current_sock = None };
    secondary =
      { socket_path = secondary_path; available = true; current_sock = None };
...
let run_in_process broker request =
  ...
  let token_count, digest, _process_ns =
    Real_processor.run request.Ipc.doc_content broker.arena
  in
...
let process_with_hedging broker request =
  ...
  if
    Int64.to_float in_process_result.process_ns
    > float broker.hedge_timer_ms *. 1_000_000.0
  then broker.hedge_count <- broker.hedge_count + 1
...
let hedged_call broker ~(input : bytes) ~(hedge_ms : int) : svc_result =
  ...
  let primary_result =
    run_in_process broker
      { Ipc.doc_len = Bytes.length input; doc_content = input }
  in
  ...
  if process_ms > float hedge_ms then (
    (* Fire hedge timer and callback *)
    hedge_fire (Clock.now_ns ());
    broker.hedge_count <- broker.hedge_count + 1;
    (* In this implementation, primary is also the hedge result for
       simplicity *)
    first_reply (Clock.now_ns ());
    {
      status = 0;
      n_tokens = primary_result.token_count;
      issues_len = 0;
      origin = `H;
    })
  else (
...
let init_pool _worker_ids =
  ...
  let secondary =
    Printf.sprintf "/tmp/l0_secondary_%d.sock" (Random.int 10000)
  in
