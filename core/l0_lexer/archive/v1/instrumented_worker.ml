...
type t = {
  fd : Unix.file_descr;
  arenas : arenas;
  mutable slowest_traces : (float * float * float * float * int * string) list;
}
...
let _alarm =
  Gc.create_alarm (fun () ->
      stats.major_cycles_since_spawn <- stats.major_cycles_since_spawn + 1)
...
let handle (w : t) ~req_id (input : bytes) =
  ...
  w.slowest_traces <-
    (trace :: w.slowest_traces
     |> List.sort (fun (t1, _, _, _, _, _) (t2, _, _, _, _, _) -> compare t2 t1)
     (* descending *)
     |> function
     | l when List.length l > 10 -> take 10 l
     | l -> l);

  (* Log if in slowest 10 so far *)
  if
    List.length w.slowest_traces <= 10
    || total_ms
       >
       let t, _, _, _, _, _ = List.nth w.slowest_traces 9 in
       t
  then
    Printf.eprintf
      "SLOW[%d]: %.3fms (prep:%.3f pretouch:%.3f process:%.3f) checksum:%08x %s\n\
       %!"
      (List.length w.slowest_traces)
      total_ms prep_ms pretouch_ms process_ms checksum
      (snd
         (List.hd
            (List.rev
               (String.split_on_char '=' (Printf.sprintf "tok=%d" n_tokens)))));
...
let rec loop (w : t) =
  ...
  | `Req (req_id, payload) ->
      handle w ~req_id payload;
      if should_retire () then exit 0;
      loop w
  | `Cancel _ -> loop w
  | `Resp _ -> loop w
  | `Hup -> exit 0
