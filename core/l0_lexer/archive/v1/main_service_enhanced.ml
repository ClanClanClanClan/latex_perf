(* Enhanced main service demonstrating SIMD.md § 3.6 timing and advanced
   features *)
...
let run () =
  ...
  let rec accept_loop () =
    ...
    let format r =
      ...
      in
      put32 0 r.Broker.status;
      put32 4 r.n_tokens;
      put32 8 r.issues_len;
      Bytes.set b 12 (match r.origin with `P -> '\001' | `H -> '\002');
      b
    in
    let send reply =
    ...
    let ms, st =
      Service_bracket.measure_bytes_in_to_reply_ready ~recv ~process ~format
        ~send
    in
    ...
    if hedged then
      Metrics_logger.bump_hedge metrics
        ~win:
          (st.Service_bracket.t_first_reply >= st.Service_bracket.t_hedge_fire);
    ...
    let row =
      {
        ...
        hedged;
        origin = (if hedged then "H" else "P");
      }
    in
    ...
    if st.t_hedge_fire <> 0L then
      ...
      Printf.eprintf "[Enhanced] %.3fms (primary: proc=%Ldns)\n%!" ms
        (Int64.sub st.t_first_reply st.t_proc_start);
