...
let create ~keep =
  { slowest = []; keep; hedged = 0; hedged_wins = 0; rotations = 0 }
...
let bump_hedge p ~win =
  p.hedged <- p.hedged + 1;
  if win then p.hedged_wins <- p.hedged_wins + 1
  ...
  try
    ...
    |> List.iter (fun (_, row) ->
           fprintf oc "%.6f,%Ld,%Ld,%Ld,%Ld,%Ld,%Ld,%b,%s\n" row.ms_total
             row.t_in_start row.t_in_done row.t_proc_start row.t_hedge_fire
             row.t_first_reply row.t_reply_ready row.hedged row.origin);
    close_out oc;
    eprintf "[Metrics] CSV dumped to %s (%d rows)\n%!" path
      (List.length p.slowest)
  with e -> eprintf "[Metrics] CSV dump failed: %s\n%!" (Printexc.to_string e)

let print_summary p =
  let hedge_rate =
    if p.hedged = 0 then 0.0 else float p.hedged_wins /. float p.hedged *. 100.0
  in
