...
let run (input : bytes) (out : Arena.buffers) : int * int * int =
  try
    ...
    let max_issues_to_count = min 200 token_count in
    (* Ultra-reduced cap to 200 for speed *)
    let i = ref 0 in
    (* Ultra-fast issue counting with early exit *)
    while !i < max_issues_to_count && !issues_len < 20 do
      (* Reduced limit to 20 for speed *)
      if Int32.compare out.Arena.issues.{!i} 0l <> 0 then incr issues_len;
      incr i
    done;
