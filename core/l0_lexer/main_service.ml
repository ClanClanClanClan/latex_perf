...
let spec_list = [
  ("--test", Arg.Set_int test_iterations, " Run performance test with N iterations");
]
...
let process_single_file filename =
  ...
  let broker = L0_lexer.Broker.create_broker "/tmp/l0_primary.sock" "/tmp/l0_secondary.sock" in
  
  let timing = L0_lexer.Service_bracket.create_timing () in
  let request = { L0_lexer.Ipc.doc_len = Bytes.length doc_content; doc_content } in
  ...
  let measurement = L0_lexer.Service_bracket.create_measurement 
    timing_complete response.token_count response.digest "in-process" in
...
let run_performance_test iterations =
  ...
  let test_doc = "\\documentclass{article}\n\\begin{document}\nHello, World!\n\\end{document}\n" in
  ...
  for i = 0 to iterations - 1 do
    ...
    measurements.(i) <- L0_lexer.Service_bracket.ns_to_ms timing_complete.reply_ready_ns -. 
  ...
  let p99 = L0_lexer.Percentiles.exact (Array.map (fun x -> x) measurements) 0.99 in
  let p999 = L0_lexer.Percentiles.exact (Array.map (fun x -> x) measurements) 0.999 in
  let mean = Array.fold_left (+.) 0.0 measurements /. float_of_int iterations in
