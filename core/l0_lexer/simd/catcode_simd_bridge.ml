...
let create_simd_table () =
  let ascii_table = Array.make 256 12 in
  (* Default to CatOther = 12 *)
  let special_mask = Bytes.create 32 in
  ...
  { ascii_table; special_mask; lookups = 0; simd_hits = 0; fallback_hits = 0 }
...
let batch_classify (input : bytes) (start : int) (len : int) : int array =
  let result = Array.make len 12 in
...
let get_stats () =
  Printf.sprintf
    "SIMD Catcode Stats: lookups=%d, simd_hits=%d (%.1f%%), fallback=%d \
     (%.1f%%)"
    simd_table.lookups simd_table.simd_hits
    (if simd_table.lookups > 0 then
       100.0 *. float simd_table.simd_hits /. float simd_table.lookups
     else 0.0)
    simd_table.fallback_hits
    (if simd_table.lookups > 0 then
       100.0 *. float simd_table.fallback_hits /. float simd_table.lookups
     else 0.0)
...
module CatcodeBridge = struct
  ...
  let verify_simd_table () =
    ...
    List.iter
      (fun (char_code, expected_catcode) ->
        let actual = lookup_catcode char_code in
        if actual <> expected_catcode then (
    ...
    if !errors = 0 then (
      ...
      Printf.printf
        "✅ Table contains %d ASCII entries with %d special char bits\n"
...
let initialize () =
  ...
  else Printf.printf "   - ❌ Table verification: FAILED\n"
