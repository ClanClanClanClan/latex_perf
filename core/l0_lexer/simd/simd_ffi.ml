...
let catcode_to_int = function
  ...
  | Newline -> 5 | Space -> 10 | Letter -> 11 | Other -> 12 | Comment -> 14

let int_to_catcode = function
  | 0 -> Escape | 1 -> BeginGrp | 2 -> EndGrp | 3 -> Math
  | 5 -> Newline | 10 -> Space | 11 -> Letter | 14 -> Comment
  | _ -> Other
...
let create_simd_buffer capacity =
  ...
  {
    kinds = mk (); codes = mk (); start_pos = mk ();
    end_pos = mk (); lines = mk (); cols = mk ();
    count = 0; capacity = capacity;
  }

(* Clear SIMD buffer *)
let clear_simd_buffer buf =
  buf.count <- 0
...
external simd_tokenize_c : 
  ...
  (int32, int32_elt, c_layout) Array1.t ->
  int -> int
  = "simd_tokenize_stub_bytecode" "simd_tokenize_stub"
...
external simd_vs_scalar_benchmark_c :
  ...
  (int32, int32_elt, c_layout) Array1.t ->
  int -> int
  = "simd_vs_scalar_benchmark_stub_bytecode" "simd_vs_scalar_benchmark_stub"
...
let simd_tokenize input_ba buf =
  ...
  let token_count = simd_tokenize_c 
    input_ba input_len
    buf.kinds buf.codes buf.start_pos buf.end_pos buf.lines buf.cols
    buf.capacity
  in
  if token_count >= 0 then (
    ...
    failwith "SIMD tokenization failed"
  )
...
let benchmark_simd_vs_scalar bytes capacity =
  ...
  let input_len = Array1.dim input_ba in
  let diff = simd_vs_scalar_benchmark_c
    input_ba input_len
    simd_buf.kinds simd_buf.codes simd_buf.start_pos 
    simd_buf.end_pos simd_buf.lines simd_buf.cols
    scalar_buf.kinds scalar_buf.codes scalar_buf.start_pos
    scalar_buf.end_pos scalar_buf.lines scalar_buf.cols
    capacity
  in
...
(* 
let simd_buffer_to_soa buf existing_soa =
  (* Copy SIMD results to existing SoA structure *)
  let count = min buf.count existing_soa.n in
  for i = 0 to count - 1 do
    let kind = Array1.unsafe_get buf.kinds i |> Int32.to_int in
    let code = Array1.unsafe_get buf.codes i |> Int32.to_int in
    let start_pos = Array1.unsafe_get buf.start_pos i |> Int32.to_int in
    let end_pos = Array1.unsafe_get buf.end_pos i |> Int32.to_int in
    let line = Array1.unsafe_get buf.lines i |> Int32.to_int in
    let col = Array1.unsafe_get buf.cols i |> Int32.to_int in
    
    Array1.unsafe_set existing_soa.kind i (Int32.of_int kind);
    Array1.unsafe_set existing_soa.char_code i (Int32.of_int code);
    Array1.unsafe_set existing_soa.start_pos i (Int32.of_int start_pos);
    Array1.unsafe_set existing_soa.end_pos i (Int32.of_int end_pos);
    Array1.unsafe_set existing_soa.line i (Int32.of_int line);
    Array1.unsafe_set existing_soa.col i (Int32.of_int col)
  done;
  existing_soa.n <- count;
  count
*)

(* Architecture detection *)
let simd_architecture () =
  (* This would normally use cpuid or similar, but for now return based on compilation *)
    ...
    (match Sys.os_type with
     | "Unix" -> 
       (try 
          ...
          else if Str.string_match (Str.regexp ".*arm.*\\|.*aarch.*") arch 0 then "NEON"
          else "Scalar"
        with _ -> "Scalar")
     | _ -> "Scalar")
  | _ -> "Scalar"
...
(* Performance info *)
let simd_info () =
  Printf.sprintf "SIMD Architecture: %s, SIMD Width: %s" 
    (simd_architecture ())
...
(*
let tokenize_with_simd_acceleration input_bytes existing_soa =
  let simd_buf, token_count = tokenize_bytes_simd input_bytes existing_soa.capacity in
  let copied_count = simd_buffer_to_soa simd_buf existing_soa in
  copied_count
*)