...
type simd_capability = No_SIMD | SSE2 | AVX2 | AVX512 | NEON
...
module SIMD_Lexer = struct
  type simd_arena = { data : bytes; capacity : int; length : int }

  (* Stub for C-based SIMD lexer *)
  external simd_lex_avx2 : bytes -> int -> simd_arena
    = "caml_simd_lex_avx2_stub"

  external simd_lex_neon : bytes -> int -> simd_arena
    = "caml_simd_lex_neon_stub"

  external simd_lex_scalar_fallback : bytes -> int -> simd_arena
    = "caml_simd_lex_scalar_stub"
...
let compare_simd_vs_scalar input_file iterations =
  ...
  (try
     ...
     printf "SIMD total: %.3f ms (%.3f ms per iteration)\n" simd_time
       (simd_time /. float_of_int iterations)
   with
  ...
  for i = 0 to iterations - 1 do
    ...
    let exit_code =
      Sys.command
        (sprintf
           "../../../latex_perfectionist_cli --summary %s > /dev/null 2>&1"
           input_file)
    in
    let end_time = Unix.gettimeofday () in

    if exit_code = 0 then scalar_times.(i) <- (end_time -. start_time) *. 1000.0
    else scalar_times.(i) <- 999.0 (* Mark as failed *)
  done;

  let scalar_mean =
    Array.fold_left ( +. ) 0.0 scalar_times /. float_of_int iterations
  in
  ...
  let projected_1mb =
    scalar_mean *. (1100.0 /. (float_of_int file_size /. 1024.0))
  in
  printf "Projected 1.1MB scalar: %.1f ms\n" projected_1mb;

  if projected_1mb <= 20.0 then printf "Tier A compliance: ✅ ON TRACK\n"
  else printf "Tier A compliance: ❌ AT RISK (%.1f ms > 20ms)\n" projected_1mb;
...
let () =
  ...
  try compare_simd_vs_scalar input_file iterations with
  | Failure msg ->
      printf "Error: %s\n" msg;
      exit 1
  | Sys_error msg ->
