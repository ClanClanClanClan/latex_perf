...
let default = { strip_controls = ["textbf"; "emph"; "section"]; bfseries_until_brace = true; controls = [ ("section", {arity=1}); ("textbf",{arity=1}); ("emph",{arity=1}) ] }
...
let load path : config =
  try
    ...
    let ctrls =
      j |> member "controls" |> to_assoc |> List.map (fun (k,v) ->
