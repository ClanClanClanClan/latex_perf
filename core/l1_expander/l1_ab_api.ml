...
type tok = Control of string | Text of string

let tokenize_controls (s : string) : tok list =
  let n = String.length s in
  let rec take_ident i =
    if
      i < n && ((s.[i] >= 'a' && s.[i] <= 'z') || (s.[i] >= 'A' && s.[i] <= 'Z'))
    then take_ident (i + 1)
    else i
  in
  let rec loop i acc =
...
let summarize (toks : tok list) : int * int =
  List.fold_left
    (fun (c_ctrl, c_text) -> function
      | Control _ -> (c_ctrl + 1, c_text)
      | Text _ -> (c_ctrl, c_text + 1))
    (0, 0) toks
...
let tokenize_b (s : string) : tok list =
  (* Variant strategy: treat control names case-insensitively by normalizing to
     lowercase. Text chunks are left intact. *)
  let toks = tokenize_controls s in
  List.map
    (function
      | Control name -> Control (String.lowercase_ascii name) | Text t -> Text t)
    toks
