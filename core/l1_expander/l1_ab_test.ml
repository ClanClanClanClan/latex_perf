(* Minimal L1 expander A/B test driver. Intentionally simple: prints a
   placeholder and exits 0. Extend in a feature branch to compare two expander
   strategies on inputs. *)
...
let () =
  ...
  let rec loop i =
    if i >= Array.length Sys.argv then ()
      ...
      | s when !file = None ->
          ...
          loop (i + 1)
      | _ ->
          prerr_endline "usage: l1_ab_test [--csv PATH] FILE";
          exit 2
  in
  loop 1;
  let file =
  ...
  Printf.printf
    "[l1-ab] %s: A(controls=%d text=%d) B(controls=%d text=%d) equal=%b\n%!"
  ...
  | Some path ->
      ...
      if in_channel_length (open_in path) = 0 then
        output_string oc "file,ctrl_a,text_a,ctrl_b,text_b,equal\n";
      Printf.fprintf oc "%s,%d,%d,%d,%d,%b\n" file c_ctrl_a c_text_a c_ctrl_b
        c_text_b eq);
  if not eq then
    ...
    let module T = struct
      ...
      let to_str = function
        | Control n -> Printf.sprintf "\\%s" n
        | Text t -> Printf.sprintf "T(%s)" t
    end in
    let rec find_i i a b =
      match (a, b) with
      | [], [] -> None
      | _ :: _, [] | [], _ :: _ -> Some i
      | x :: xs, y :: ys -> if x = y then find_i (i + 1) xs ys else Some i
    in
    ...
    | Some i ->
        let slice lst =
          ...
          let rec drop n l =
            if n = 0 then l
            else match l with [] -> [] | _ :: xs -> drop (n - 1) xs
          in
