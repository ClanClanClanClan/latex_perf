...
module L1MacroTable = struct
  ...
  let macro_table =
    [|
      ...
      {
        pattern = "\\texttt";
        expansion = StyleCommand "MONOSPACE";
        priority = 2;
      };
      {
        pattern = "\\textsc";
        expansion = StyleCommand "SMALLCAPS";
        priority = 2;
      };
      (* Punctuation *)
  ...
  let macro_hash =
    ...
    Array.iter (fun entry -> Hashtbl.add h entry.pattern entry) macro_table;
    h

  let find_macro pattern =
    try Some (Hashtbl.find macro_hash pattern) with Not_found -> None
end
...
module ExpansionIterator = struct
  ...
  let create_iterator soa =
    {
      soa;
      scratch = ScratchRing.create_scratch_ring (64 * 1024);
      (* 64KB scratch *)
      position = 0;
      in_expansion = false;
      expansion_queue = [];
  ...
  let extract_macro_pattern soa index =
    if index >= soa.n then None
      ...
      if catcode = 0 then
        ...
        if length > 0 && length < 32 then
          (* reasonable macro length *)
  ...
  let expand_macro_to_synthetic iter entry =
    ...
    | L1MacroTable.SimpleText text -> (
        ...
        | Some (offset, len) -> [ SyntheticToken (offset, len, "simple_text") ]
        | None -> [] (* Scratch full *))
    ...
    | L1MacroTable.MathSymbol symbol -> (
        ...
        | Some (offset, len) -> [ SyntheticToken (offset, len, "math_symbol") ]
        | None -> [])
