    ...
    | `Assoc props ->
        ...
        List.map
          (fun j ->
            ...
            let expansion =
              j |> member "expansion" |> to_list |> List.map token_of_json
            in
            let mode = j |> member "mode" |> to_string |> mode_of_string in
            let category =
              j
              |> member "category"
              |> to_string_option
            ...
            let expand_in_math =
              j
              |> member "expand_in_math"
              |> to_bool_option
              |> Option.value ~default:true
            in
            let expand_in_text =
              j
              |> member "expand_in_text"
              |> to_bool_option
              |> Option.value ~default:true
            in
            let non_expandable =
              j
              |> member "non_expandable"
              |> to_bool_option
              |> Option.value ~default:true
            in
            {
...
(* Unified interface *)
type unified_macro = Symbol of V25R2.entry | Argumentful of ArgSafe.macro

let load_all () =
  let symbols =
    try V25R2.load "macro_catalogue.v25r2.json" |> List.map (fun e -> Symbol e)
    with _ -> []
  in
  let argumentful =
    try
      ArgSafe.load "macro_catalogue.argsafe.v25r1.json"
