...
type catalogue = { id : string; version : string; macros : macro list }
...
let string_member key = function
  | `Assoc kv -> (
      try match List.assoc key kv with `String s -> s | _ -> raise Not_found
      with Not_found -> failwith ("missing string " ^ key))
  | _ -> failwith "expected object"

let opt_string_member key = function
  | `Assoc kv -> (
      try
        match List.assoc key kv with
        | `String s -> Some s
        | `Null -> None
        | _ -> None
      with Not_found -> None)
...
let bool_member key = function
  | `Assoc kv -> (
      try match List.assoc key kv with `Bool b -> b | _ -> raise Not_found
      with Not_found -> failwith ("missing bool " ^ key))
  | _ -> failwith "expected object"

let int_member key = function
  | `Assoc kv -> (
      try match List.assoc key kv with `Int i -> i | _ -> raise Not_found
      with Not_found -> failwith ("missing int " ^ key))
  | _ -> failwith "expected object"

let list_member key = function
  | `Assoc kv -> (
      try match List.assoc key kv with `List xs -> xs | _ -> raise Not_found
      with Not_found -> failwith ("missing array " ^ key))
  | _ -> failwith "expected object"

let assoc_member key = function
  | `Assoc kv -> (
      try List.assoc key kv
      with Not_found -> failwith ("missing object " ^ key))
  | _ -> failwith "expected object"
...
let parse_macro (j : Yojson.Safe.t) : macro =
  ...
  let aliases =
    match j with
    | `Assoc kv -> (
        match List.assoc_opt "aliases" kv with
        | Some (`List xs) ->
...
let eps_inline_safe (s : string) : bool =
  (* very simple checker: only braces, spaces, $1..$3, and the allowed control
     sequences *)
  ...
  let rec check pos =
    ...
    then check (pos + 1)
      ...
      if List.mem cs allowed_inline_css then check (Str.match_end ()) else false
    else false
  in
  check 0
