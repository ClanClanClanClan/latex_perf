(* Minimal brace-aware expander for two controls: \textbf{...}, \emph{...}
   Non-nested, best-effort. Returns expanded string by stripping the control
   and braces. *)
...
let expand_once_with cfg s =
  ...
  while !i < n do
    if s.[!i] = '\\' then (
      ...
      match name with
      | _ when (try let p = List.assoc name cfg.Catalogue_loader.controls in p.Catalogue_loader.arity >= 1 with Not_found -> List.mem name cfg.Catalogue_loader.strip_controls) ->
          (* Consume 'arity' brace blocks and concatenate their contents *)
          let arity = try (snd (List.find (fun (n,_) -> n = name) cfg.Catalogue_loader.controls)).arity with Not_found -> 1 in
          let rec collect k idx =
            if k = 0 then (Buffer.contents b, idx) else
            match find_brace_block s idx with
            | Some (off,len) -> Buffer.add_substring b s off len; collect (k-1) (idx + len + 2)
            | None -> (Buffer.contents b, idx)
          in
          let start_len = Buffer.length b in
          let _, idx' = collect arity j in
          if Buffer.length b = start_len then (Buffer.add_char b s.[!i]; incr i) else i := idx'
