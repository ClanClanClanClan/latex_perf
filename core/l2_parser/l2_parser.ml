...
type arg = Optional of inline list | Required of inline list
...
let[@inline] peek state =
  if state.pos < Array.length state.tokens then Some state.tokens.(state.pos)
  else None
...
let[@inline] advance state = state.pos <- state.pos + 1
...
let error state msg =
  ...
  if state.errors > state.options.max_errors then
    failwith
      (Printf.sprintf "Too many parse errors (>%d)" state.options.max_errors)

(* Forward declarations *)
let rec parse_blocks state = parse_block_list state []
...
and parse_block state =
  ...
  | Some (TGroupOpen | TMacro _ | TChar _), _ -> parse_paragraph state
...
and parse_environment state =
  ...
  | Some (TMacro "begin") ->
      ...
      if name = "document" && state.in_preamble then state.in_preamble <- false;
...
and parse_env_name state =
  match parse_argument state with [ Text name ] -> name | _ -> "unknown"
...
and parse_env_body state name =
  let rec loop acc =
    ...
    | Some (TMacro "end"), Some TGroupOpen ->
        ...
        if end_name = name then List.rev acc
        else (
          error state
            (Printf.sprintf "Mismatched \\end{%s}, expected \\end{%s}" end_name
               name);
          List.rev acc)
    | None, _ | _, None ->
...
and parse_paragraph state =
  ...
  if inlines = [] then None else Some (Paragraph inlines)
...
and parse_include state cmd =
  ...
  advance state;
  (* consume command *)
  let path = match parse_argument state with [ Text p ] -> p | _ -> "" in
...
and parse_inline state =
  ...
  | None -> None
  | Some (TChar (c, cat))
    when cat <> Space && cat <> EndLine && cat <> MathShift ->
  ...
  | Some (TChar (c, MathShift)) when Uchar.to_int c = Char.code '$' ->
      ...
      state.math_mode <- true;
      let content =
        parse_math_until state (TChar (Uchar.of_char '$', MathShift))
      in
      state.math_mode <- old_math;
...
and parse_optional_arg state = parse_until state (TMacro "]")
and parse_group_content state = parse_until state TGroupClose
...
and parse_math_content state = parse_inlines state
and parse_math_until state end_tok = parse_until state end_tok
  ...
  let state =
    {
      ...
      math_mode = options.math_mode;
      in_preamble = false;
      (* Start in body for simple documents *)
    }
  in

  let preamble = [] in
  (* No preamble for simple documents *)
  let body = parse_block_list state [] in

  let parse_time = (Sys.time () -. start_time) *. 1000. in
...
let rec pp_inline fmt = function
  | Command { name; args } -> Format.fprintf fmt "\\%s%a" name pp_args args
  | Text s -> Format.fprintf fmt "\"%s\"" s
  | MathInline content -> Format.fprintf fmt "$%a$" (pp_list pp_inline) content
  | Group content -> Format.fprintf fmt "{%a}" (pp_list pp_inline) content
  | Space -> Format.pp_print_string fmt "␣"
  | Newline -> Format.pp_print_string fmt "⏎"
  | InlineError { msg; _ } -> Format.fprintf fmt "ERROR(%s)" msg
...
let rec pp_block fmt = function
  | Section { level; title; content } ->
      Format.fprintf fmt "Section(L%d, %a, [%a])" level (pp_list pp_inline)
        title (pp_list pp_block) content
  | Environment { name; args; body } ->
      Format.fprintf fmt "Env(%s%a, [%a])" name pp_args args (pp_list pp_block)
        body
  | Paragraph content ->
  ...
  | Include { kind; path } ->
      Format.fprintf fmt "Include(%s, %s)"
        ...
        path
  | BlockError { msg; _ } -> Format.fprintf fmt "ERROR(%s)" msg
...
let pp_document fmt doc =
  ...
  Format.fprintf fmt
    "  stats: { tokens=%d; time=%.2fms; errors=%d; depth=%d }@\n"
    doc.stats.total_tokens doc.stats.parse_time_ms doc.stats.error_count
    doc.stats.max_depth;
  Format.fprintf fmt "}"
