(* L2 Parser - Packed Token Version Parses packed tokens from L1 without
   conversion overhead *)
...
let create_parser_state tokens =
  { pos = 0; tokens; error_count = 0; max_depth = 0; current_depth = 0 }
...
let get_macro_name token =
  ...
  else failwith "Not a macro token"
...
let get_char token =
  if is_char_token token then
    ...
    Char.chr ascii
  else failwith "Not a character token"
      ...
      if name = "[" then (
        ...
        let rec parse_until_close () =
          ...
          | Some token when is_char_token token ->
              ...
              parse_until_close ()
          | Some _ -> parse_until_close () (* Skip other tokens *)
          | None ->
              [
...
let rec parse_blocks state =
  ...
  while state.pos < Array.length state.tokens do
    ...
    if display_math <> [] then blocks := display_math @ !blocks
    else
      (* Parse regular text *)
      let text_content = parse_text_content state in
      if text_content <> [] then blocks := Paragraph text_content :: !blocks
...
let rec ast_to_string blocks =
  let rec block_to_string = function
    | Paragraph inlines ->
        "Paragraph("
        ^ String.concat "" (List.map inline_to_string inlines)
        ^ ")"
    | MathDisplay inlines ->
        "MathDisplay("
        ^ String.concat "" (List.map inline_to_string inlines)
    ...
    | Environment { name; _ } -> "Environment(" ^ name ^ ")"
    | BlockError { msg; _ } -> "BlockError(" ^ msg ^ ")"
    | _ -> "OtherBlock"
