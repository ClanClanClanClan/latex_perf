...
### CVE Scanning (spec W50)

Automated vulnerability scanning via `.github/workflows/cve-scan.yml`:

- **Trivy** filesystem scan on every push to main and weekly (Monday 6am UTC)
- Fails on CRITICAL or HIGH severity CVEs with available fixes
- Generates CycloneDX SBOM as CI artifact (90-day retention)
- Rust dependencies scanned separately via `Cargo.lock`

### Supply Chain

- **SBOM**: CycloneDX JSON generated by Trivy in CI (`cve-scan.yml`)
