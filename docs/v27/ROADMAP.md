...
> **Version-of-record:** `dune-project` = **v27.1.61**. Tags `v27.1.58`–`v27.1.60` are on `origin/main` (v27.1.60 = the structural-fatal compile-gate, merged #498). **v27.1.61 recovers the #498 squash-stranded commits** (growable-stack + single-source `is_compile_blocking` hardening, the `check_roadmap_facts` anti-drift gate, and this ROADMAP v3) — the squash-strand trap fired a second time; see the guardrail in §1.
