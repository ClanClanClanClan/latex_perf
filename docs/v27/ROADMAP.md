...
> **Version-of-record:** `dune-project` = **v27.1.63** (2026-09-12). Tags `v27.1.58`–`v27.1.62` are on `origin/main`. **v27.1.63 is the first release graded against REAL DOCUMENTS**: PRs #513–#595, 85 commits, 47 days. It publishes an out-of-sample number for the first time — the VIRGIN sample (ranks 201–400, never used to tune anything) and the tuned sample are both in `PROJECT_STATE.md`'s generated block, which is the authority; this line deliberately restates neither, because a positional number copied into ROADMAP prose is exactly how the last three of them went stale (C-40, OPEN-078). Its headline corrections: the shipped verdict no longer cites a compile theorem (Phase A, #575); five `--apply-fixes` producers were found destroying real compiling papers and two were WITHDRAWN outright (`TYPO-038`, `TYPO-028`); the real-paper fixer differential finally has an artefact and a blocking gate (#595, OPEN-071); and `model_fatal_iff` landed as the sixth capstone (#592, D1). ⚠ **Read `docs/v27/PROJECT_STATE.md` for where the project stands — not this line.** **v27.1.62 ships the whole round-7 fix train**: the #501 glue train (verdict-wiring/size-cap/feature-scanner/gate-linearisation), the #503 regression corpus + monotone gate, and the eight fix trains #504–#511 — taking the known false-READY **fixture baseline from twenty-one down to seven**, with the error-halt class empty AT THAT TIME. **Both halves of that sentence are now superseded and are kept only as a record of what v27.1.62 shipped:** the fixture baseline stands at **seventeen** (4 strong-fatal, 13 error-halt; 15 accept-pins excluded), and the error-halt class is NOT empty. ⚠ This read **eight (seven strong-fatal, one error-halt)** until 2026-09-20 — understating the in-house known-unsound set by 2.1x AND inverting its split. `check_roadmap_facts.py` could not see it: `load_false_ready_count` reads corpus (a)'s `KNOWN_FALSE_READY` allowlist in `diff_compile_check.sh` to check a claim about corpus (b), `corpora/false_ready/manifest.json` — the (a)/(b) conflation this document calls the project's most-repeated error, embedded inside the gate written to prevent it (C-60). Ground truth is the manifest: `[x for x in fixtures if x['expected_cli']=='READY' and x['pdflatex']!='compiles']`. The rise is measurement, not regression — `fr_thmtools_redeclare` and `fr_thmrestate_counter` record a theorem-counter collision that the first real-paper differential (`corpora/real_roots`) found to account for **eight of the eleven cardinal-bug verdicts in a 200-paper arXiv sample** — the largest single source of them on real documents. (Three distinct quantities are in play and must not be conflated: the **allowlist 8** over the 65-doc differential corpus, the **fixture baseline 17** over `corpora/false_ready`, and the **eleven** measured over 200 real arXiv roots.) It was always there; nothing had measured it. This supersedes the earlier statements that v27.1.62 would carry the glue train alone or R7-2 alone. Per the no-SHA header policy (round 4), commit hashes live in git, not here.
...
2. **PRECISE, LOCALIZED, EXPLAINED diagnosis + safe fix.** pdflatex says "it failed" cryptically, often at the *wrong* line. We say exactly **what**, **where**, and **why**, and can **suggest or apply a guard-gated fix**. This is arguably the *stronger* wedge — it leverages the whole 660-rule engine + `--explain` + **164** fix-producers + **124** candidates, none of which a compiler has.
...
⚠ **REDEFINED BY [ADR-012](adr/ADR-012-contract-bounded-proven-tier.md) (2026-09-26).** A *proven* verdict now means a verdict of the **contract-bounded strict tier**: Turing-free text across the whole project closure, whose exact configuration has a contract generated and solo-attested under the pinned TeX Live, decided exactly (READY iff compiles, both directions) by a Coq-proved decider. The North Star is **strict-tier coverage at zero `strict_wrong` on a VIRGIN sample**, where `strict_wrong` also counts a wrong reason or location. The old "premise-certified" figure is published **only** as a heuristic-tier statistic. In milestone M0 no verdict is proven, so the North-Star figure is zero until the Coq kernel ships (M2/M3). Programme: ledger row OPEN-116; design: [STRICT_TIER_DESIGN.md](STRICT_TIER_DESIGN.md).

⚠ **THIS DOCUMENT NO LONGER PUBLISHES THE NUMBER.** It restated a 65-doc differential banner (`35/65 = 53.8%`) that disagreed with its own confusion matrix eighty lines below and with `COMPILATION_GUARANTEE.md`, and the metric it named was never measured on real papers at all. The metric is now GENERATED into `PROJECT_STATE.md` §1 ("Strict-tier coverage — THE North-Star metric (ADR-012)") from committed per-document artefacts, and `check_project_state.py` diffs it on every PR. **Read it there; do not restate it here.** **Target:** grow the numerator (E-track subset extension + V1 modelling) while driving false-READY **allowlist → 0**, on a *hash-manifested, version-controlled* corpus (S0/S4), and (for the flagship) parameterize the promise by **engine + TeX-Live year**.
...
- **180 proof files total** = **65 core + 114 generated + 1 ML** (plus **7 archived** `.disabled` files, tracked separately) (`governance/project_facts.yaml`). **NO `Admitted`, NO `Axiom`** in active proofs (only archived `.disabled` files carry them).
...
- `theorem_count_reported: 1599`; `per_rule_soundness_count: 643`. ⚠ **Both are CONSTRUCTED, not measured** — see `governance/project_facts.yaml`'s `honesty_annotation` key: the theorem count is a raw `Theorem`/`Lemma`/`Corollary` grep in which 803 of the 804 theorems under `proofs/generated` share one byte-identical proof body over 316 distinct predicates, 57 of them `:= false`; and `per_rule_soundness_count` is the non-reserved rule headcount copied verbatim. OPEN-014/OPEN-051 track replacing them.

### Coverage numbers
- **Rule catalogue:** 660 total / 17 reserved / **643 non-reserved / 643 shipped** (`governance/project_facts.yaml`, internally consistent).
- **Fix producers (Bucket A):** **164 (~25%) — SETTLED, authoritative** (`FIX_PRODUCER_LEDGER.md`, `SHIPPED_VERSIONS`). Decomposition: 67 (L1+ via `mk_result_with_fix`; read 70 until 2026-09-20 — the delta is TYPO-028, TYPO-038 and HI-001, the three producers WITHDRAWN for corrupting real documents, and `gen_candidate_backlog.py --check` now runs in required `spec-drift` so it cannot drift again) + 47 (L0 via `with_fix`/`fix_of`) + the remainder across families. **Bucket A = 466 rules; 164 shipped / 302 pending.** **⚠ Do NOT "correct" 164 down to 70** — the 70-count is only the `_with_fix_exempt`-grep subset (verbatim-hardened L1+), not the producer total (see C-DR1). **Cadence commitment:** ≥10 new Bucket-A producers per patch release (`FIX_PRODUCER_LEDGER.md:781`) — actual recent cadence ~1/patch (audit-depth over throughput).
...
`--compile-check` runs **T0 (parse + LP-Foreign classify) + T2 (source closure) + T3 (feature/engine compat) + T4 (aux label uniqueness) + T5 (36 DELIM/ENC/PRT Error rules; OPEN-058 measures the EFFECTIVE belt at 12 of them) + the v27.1.60 structural-fatal gate**, with **T1 (macro expansion) a deliberate conservative no-op**. Scoped to **LP-Core**, **pdflatex only** (see S-ENGINE), for the READY oracle defined in S-ORACLE.
...
- **34 true-READY / 20 true-NOT-READY / 8 false-READY / 3 false-NOT-READY** (total 65). *(Re-measured 2026-07-28; was 35/…/2. `fail_duplicate_label` moved to over-rejection because #501 made the model verdict a hard exit-code conjunct — the safe direction, and a consequence of that fix rather than a regression. See COMPILATION_GUARANTEE.md.)*
...
Real-doc pdflatex/Overleaf is **5–60+ s** (multi-pass + biber + tikz + network). Our **13.7 s @ 316 KB is the COLD one-shot CLI** (~55 ms startup dominates small docs); the **warm kernel is 62 / 337 ms**. **We win categorically on the DECIDABLE class delivered PER-KEYSTROKE, NOT by racing pdflatex on completeness.** **⚠ Remeasured 2026-08-02, and the two halves of this bullet have aged differently.** The **WARM** figure held up: 337 ms @316 KB then, ~344 ms @300 KB now. The **COLD `--compile-check`** figures are v27.1.57, taken BEFORE the R1 fast kernel shipped in v27.1.59, and they now UNDERSELL the tool by roughly an order of magnitude — 13.7 s @316 KB then, **784 ms @300 KB** now (idle CI runner, full process including startup). They were also hardcoded prose with no generated source, which is why the staleness survived four releases. The baseline is now a tracked artefact: run `scripts/bench_wedge.sh` (R-BENCH) for COLD and WARM columns at every size band, and see `corpora/perf/keystroke_budget.json` for the recorded numbers `check_keystroke_budget.py` (R-BUDGET) ratchets against. Quote those, not this paragraph. The finding that matters and that no amount of measurement noise changes: **`parse` is not the bottleneck** (about 4% of the warm kernel, comfortably inside its budget) — **rule execution is**, linearly at roughly 1.1 ms/KB, leaving the `rules` stage about an order of magnitude over its 30 ms budget at 300 KB. Note also that the required `edit_window_gate.sh` does NOT measure any stage of that budget: it times `Real_processor.run`, the L0 lexer, on a random 4096-byte slice, and reports p95 = 0.031 ms on the same 300 KB document whose `rules` stage misses by ~11x.
...
| **R-BENCH** | Cold+warm wedge bench, banked | **SHIPPED** | S | none | `scripts/bench_wedge.sh` reports COLD and WARM columns at every size band (4/50/100/300 KB, deterministic line-aligned slices of `perf_smoke_big.tex`, so no duplicate corpus is committed). Runs informationally in `perf-ci`. COLD = full CLI process incl. ~55 ms startup, which is what users actually get, because R2/R-WARM is still NOT STARTED; WARM = `bench_readiness_kernel` with startup excluded, which is what the per-keystroke claim rests on. |
| **R-BUDGET** | Keystroke budget invariant gate | **SHIPPED (as a regression ratchet, not a spec-conformance gate — see below)** | S–M | R-BENCH | `scripts/tools/check_keystroke_budget.py` + `bench_readiness_kernel.ml` (excludes startup), running in `perf-ci`. **MEASURED ON ARRIVAL, and the per-stage budget above is NOT met — on every machine tried.** Warm, startup excluded, at 300 KB. `parse` PASSES everywhere (10.5 ms on the CI runner, ~6.8 ms on an idle Apple-Silicon laptop, against a 40 ms budget) and is only ~4% of the kernel. `rules` misses everywhere: **284.0 ms (9.5x over) on the idle `ubuntu-latest` CI runner**, **~159 ms (5.3x over) on an idle developer laptop**. Quote the pair, not one number: the hardware spread is ~1.8x, so a single figure implies a precision that does not exist. The gate baselines against CI because CI is the only environment that is reproducibly idle — `corpora/perf/keystroke_budget.json` records which run it came from. Rule execution is linear at ~1.1 ms/KB, the signature of every rule re-scanning the whole document; that is R-SHARED's target and it is why the kernel misses. The gate therefore ships as a MONOTONE ratchet against a recorded baseline plus a tracked gap-to-budget, in the idiom of the false-READY and apply-fixes manifests: it lands green, fails on a regression, and fails if the gap WIDENS. A red-on-arrival required gate would block every merge. It also covers the BATCH surface (one full-lint pass, same idle runner: 0.11 s @4 KB, 1.11 s @50 KB, 2.34 s @100 KB, 8.23 s @300 KB), which Principle 9 requires a budget for and never had — no target is invented, the measured value is recorded and regression forbidden. ⚠ `shared` / `structural` / `IPC` have no separate instrumentation in the bench, so only `parse` and `rules` are graded; extending the bench is the follow-up. ⚠ The shipped baseline was recorded on a loaded developer machine and is marked `provisional`, which makes the ratchet LOOSE until re-recorded on an idle runner; the gate now REFUSES to record above a load threshold so this cannot recur silently. |
...
| **R-PARALLEL** | OCaml-5 Domain fan-out (CONDITIONAL — measure first) | NOT STARTED — **conditional now RESOLVED by measurement: pursue** | M | R-BUDGET (measure) | **The abandon-condition was `rules_ms` already < 10 ms; R-BUDGET measured 284.0 ms @ 300 KB on an idle runner, so it is not met and this item stays live.** Sequence it AFTER R-SHARED, though: rule execution is linear at ~1.1 ms/KB because every rule re-scans the whole document, and fanning that waste across domains buys a constant factor where removing it buys the slope. If pursued, Domain fan-out over rule families; **watch `Str` globals** (not domain-safe). |
...
| **C-DR1** | Document the SETTLED 164 producer decomposition + cross-ref the two ledgers | OPEN (doc-only) | S | none | **Producer count SETTLED at 164** = 70 (`mk_result_with_fix`) + 47 (`with_fix`/`fix_of`) + rest. Document the decomposition + cross-reference `FIX_PRODUCER_LEDGER.md` + backlog. **Do NOT "correct" 164 down to 70.** The **"596" figure is a phantom** (appears nowhere; ignore it). |
...
1. ~~**R7-INFRA-1 + R7-INFRA-2 — fixtures + monotone gate + pdflatex differential CI.**~~ ✅ **DONE** — INFRA-1 in PR-R7-0 (#503); INFRA-2 in #516 (`.github/workflows/tex-oracle.yml`, digest-pinned TeX Live 2026 image, `REQUIRE_PDFLATEX=1`, both oracle protocols, advisory pending a soak before promotion to a required context). Original text: Commit every confirmed round-7 repro; CI fails if the false-READY count increases; pinned-TeX-Live dual-protocol oracle on every release train. **Realizes round-4's S-CI-TEX and the S0 release-gate wiring; locks in the audit's value before any fix.** *(Effort S–M.)*
2. **R7-2 — the verified input-model pre-pass** (EOL/`^^`/live-prefix). Kills the biggest belt in one train: four false-READY classes + the 37/38-roots real-project over-reject + the dead-byte perf tax. *(Coq L; rank sequencing per the execution doc.)* **Partially shipped in v27.1.62**: the add-NOT-READY-only arms landed (#504 CR/CRLF comments + no-`\end{document}`, #510 `\verb`-EOL). The remaining, dangerous half — the live-region model and its dead-region truncation — is **DWR-3**, the one train that can *introduce* a false-READY, and is scheduled last with its own adversarial pass.
...
- **R-CST / R4 — incremental CST** — LAST, only if profiling shows PARSE is the residual floor (**R-BUDGET now answers this: parse is 10.5 ms of a 294.5 ms warm kernel at 300 KB, about 4%, and well inside its 40 ms budget — so parse is NOT the floor and this item stays deferred**); **alphabet-scoped to pre-expansion**, under R-PARITY-GATE. Deferrable indefinitely. (R-PARALLEL/R-SIMD similarly contingent — measure first, abandon if `rules_ms` < 10 ms.)
...
- **R-count-authority:** 164 (ledger) is generated independently from 124 (candidates) and 70 (`_with_fix_exempt` subset); the new roadmap fact-checker (§5) closes the cross-file drift hole. C-DR1 documents the reconciliation.
