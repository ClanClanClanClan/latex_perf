...
> **Version-of-record:** `dune-project` = **v27.1.60** on the checked-out tree. Tags **`v27.1.58` and `v27.1.59` are tagged on `origin/main`** (both real, pushed). **v27.1.60** carries the structural-fatal compile-gate on worktree branch `feat/v27160-sound-compile-gate` (`/private/tmp/wt-sound`) — pending user merge/tag.
...
- The v27.1.60 structural-fatal gate closed the prior **10 → 8** false-READY residual — `fail_double_subscript` and `fail_no_documentclass` are now correctly NOT-READY (and `fail_missing_begin_document` is caught by T0, so it is not a false-READY either).

All **8** residual false-READYs need macro/package-universe modelling or full expansion (0 new beyond the documented `KNOWN_FALSE_READY` allowlist, whose 8 entries ARE this count), and the gate holds **0 over-rejection on the scanned real-root corpus** (after the found-and-fixed precision regressions above). See `docs/COMPILATION_GUARANTEE.md` for the enumerated residual.
...
| **S4** | Deepen differential validation vs real pdflatex at scale | PARTIAL (65-doc matrix + 6,396 over-reject pass) | M | user's 6,899 compiling root papers; **local** pdflatex | harness run over full real-paper tree; confusion matrix published in `COMPILATION_GUARANTEE.md`; 0 NEW false-READY; over-rejection rate reported per LP-tier |
