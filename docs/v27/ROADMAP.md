...
2. **PRECISE, LOCALIZED, EXPLAINED diagnosis + safe fix.** pdflatex says "it failed" cryptically, often at the *wrong* line. We say exactly **what**, **where**, and **why**, and can **suggest or apply a guard-gated fix**. This is arguably the *stronger* wedge — it leverages the whole 660-rule engine + `--explain` + **165** fix-producers + **124** candidates, none of which a compiler has.
...
- **Fix producers (Bucket A):** **165 (~25%) — SETTLED, authoritative** (`FIX_PRODUCER_LEDGER.md`, `SHIPPED_VERSIONS`). Decomposition: 70 (L1+ via `mk_result_with_fix`) + 47 (L0 via `with_fix`/`fix_of`) + the remainder across families. **Bucket A = 466 rules; 165 shipped / 301 pending.** **⚠ Do NOT "correct" 165 down to 70** — the 70-count is only the `_with_fix_exempt`-grep subset (verbatim-hardened L1+), not the producer total (see C-DR1). **Cadence commitment:** ≥10 new Bucket-A producers per patch release (`FIX_PRODUCER_LEDGER.md:781`) — actual recent cadence ~1/patch (audit-depth over throughput).
...
| **C-DR1** | Document the SETTLED 165 producer decomposition + cross-ref the two ledgers | OPEN (doc-only) | S | none | **Producer count SETTLED at 165** = 70 (`mk_result_with_fix`) + 47 (`with_fix`/`fix_of`) + rest. Document the decomposition + cross-reference `FIX_PRODUCER_LEDGER.md` + backlog. **Do NOT "correct" 165 down to 70.** The **"596" figure is a phantom** (appears nowhere; ignore it). |
...
- **R-count-authority:** 165 (ledger) is generated independently from 124 (candidates) and 70 (`_with_fix_exempt` subset); the new roadmap fact-checker (§5) closes the cross-file drift hole. C-DR1 documents the reconciliation.
