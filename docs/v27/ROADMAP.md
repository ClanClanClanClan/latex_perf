...
| **R-PARALLEL** | OCaml-5 Domain fan-out (CONDITIONAL — measure first) | NOT STARTED — **conditional now RESOLVED by measurement: pursue** | M | R-BUDGET (measure) | **The abandon-condition was `rules_ms` already < 10 ms; R-BUDGET measured it at ~329 ms @ 300 KB, so it is not met and this item stays live.** Sequence it AFTER R-SHARED, though: rule execution is linear at ~1.1 ms/KB because every rule re-scans the whole document, and fanning that waste across domains buys a constant factor where removing it buys the slope. If pursued, Domain fan-out over rule families; **watch `Str` globals** (not domain-safe). |
