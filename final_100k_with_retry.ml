...
let () =
  ...
  Printf.printf "=== FINAL 100K TEST WITH RETRY ===\n";
  Printf.printf "Starting service with 10ms hedge timer...\n%!";
  ...
  | 0 -> 
    ...
    execv "/Users/dylanpossamai/Library/CloudStorage/Dropbox/Work/Articles/Scripts/_build/default/latex-parse/src/main_service.exe"
          [|"main_service"; "--workers"; "2"; "--hedge-ms"; "10"|]
  | pid -> pid 
  in
  ...
  Printf.printf "\nRunning %d requests with retry on EOF...\n%!" n_total;
  Printf.printf "Expected duration: ~12-15 minutes\n\n%!";
  ...
  for i = 1 to n_total do
    if i mod 5000 = 0 then begin
      ...
      Printf.printf "[%s] %6d / %d (%.0f req/s, %.1f min elapsed, est. %.1f min remaining, %d retries, %d errors)\n%!" 
        (let t = Unix.localtime (Unix.time ()) in
         Printf.sprintf "%02d:%02d:%02d" t.tm_hour t.tm_min t.tm_sec)
        i n_total rate (elapsed /. 60.0) (remaining /. 60.0) !retries !errors
    end;
    ...
    let rec attempt retry_count =
      try
        ...
        samples := dt :: !samples
      with 
      | Failure "EOF" when retry_count < 3 ->
          (* EOF during rotation - retry after brief delay *)
          incr retries;
  ...
  Printf.printf "\n" ;
  Printf.printf "════════════════════════════════════════════════\n";
  Printf.printf "      FINAL 100K TEST WITH RETRY RESULTS        \n";
  Printf.printf "════════════════════════════════════════════════\n\n";
  
  Printf.printf "SUMMARY:\n";
  Printf.printf "  Total requests:     %d\n" n_total;
  Printf.printf "  Successful:         %d\n" (List.length !samples);
  Printf.printf "  Retries (EOF):      %d\n" !retries;
  Printf.printf "  Failed after retry: %d (%.4f%%)\n" !errors (100.0 *. float !errors /. float n_total);
  ...
  if List.length !samples > 0 then begin
    ...
    Printf.printf "════════════════════════════════════════════════\n";
    Printf.printf "REQUIREMENT: P99.9 ≤ 20ms, 0%% errors\n";
    Printf.printf "ACTUAL:      P99.9 = %.3f ms\n" p999;
    Printf.printf "ERROR RATE:  %.4f%% (after retry)\n" (100.0 *. float !errors /. float n_total);
    Printf.printf "\n";
    Printf.printf "FINAL STATUS: %s\n" 
      (if p999 <= 20.0 && !errors = 0 then 
        "✅ ✅ ✅ ALL REQUIREMENTS MET ✅ ✅ ✅"
       else if p999 <= 20.0 && !errors < 10 then 
        "⚠️  LATENCY OK, MINIMAL ERRORS AFTER RETRY" 
       else if p999 <= 20.0 then
        "❌ LATENCY OK BUT STILL HAS ERRORS"
       else 
        "❌ FAILS LATENCY REQUIREMENT");
    Printf.printf "════════════════════════════════════════════════\n"
  end else
