...
let () =
  ...
  if total <= 0 then (
    prerr_endline "[ab] iterations must be > 0";
    exit 2);
  ...
  for i = 0 to total - 1 do
    ...
    if i land 4095 = 0 then
      if
        tok < Config.ab_expected_tokens_min
        || tok > Config.ab_expected_tokens_max
      then (
        Printf.eprintf "[ab] token count out of expected range: %d\n%!" tok;
        exit 3)
  done;
  ...
  Printf.printf
    "[ab] stats min=%.3f p50=%.3f p95=%.3f p99=%.3f p999=%.3f max=%.3f (ms)\n%!"
  ...
  if n >= 100_000 then
    if p999 > Config.ab_p999_target_ms then (
      Printf.eprintf "[ab] FAIL: P99.9 = %.3f ms > %.3f ms target\n%!" p999
        Config.ab_p999_target_ms;
      exit 3)
    else
      Printf.printf "[ab] PASS: P99.9 = %.3f ms ≤ %.3f ms\n%!" p999
        Config.ab_p999_target_ms
  else Printf.printf "[ab] NOTE: N<100k, skipped strict P99.9 gate\n%!"
