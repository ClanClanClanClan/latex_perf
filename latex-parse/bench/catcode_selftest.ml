...
let expect cond msg = if not cond then (prerr_endline ("[catcode-selftest] "^msg); exit 1)
...
let test_basic () =
  ...
  expect (Catcode.classify_ascii (Char.code '\n') = 5) "NL should be Newline (5)";
  ...
  expect (Catcode.classify_ascii (Char.code ' ') = 10) "space should be Space (10)";
  expect (Catcode.classify_ascii (Char.code '~') = 13) "~ should be Active (13)";
  expect (Catcode.classify_ascii (Char.code '%') = 14) "% should be Comment (14)";
  expect (Catcode.classify_ascii 127 = 15) "DEL should be Invalid (15)";
  ()

let test_letters () =
  for c = Char.code 'A' to Char.code 'Z' do
    expect (Catcode.classify_ascii c = 11) "uppercase letter not classified as Letter (11)"
  done;
  for c = Char.code 'a' to Char.code 'z' do
    expect (Catcode.classify_ascii c = 11) "lowercase letter not classified as Letter (11)"
  done
...
let () =
  test_basic (); test_letters (); test_invariants ();
