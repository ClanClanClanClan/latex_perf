...
let usage () =
  prerr_endline
    "usage: edit_replay_bench FILE EDITS [--warmup N] [--window BYTES] [--csv \
     PATH]";
  ...
  let rec loop i =
    if i >= Array.length Sys.argv then ()
      ...
      | "--mode" ->
          ...
          | _ ->
              prerr_endline "[replay] invalid --mode";
              usage ());
          loop (i + 2)
      | "--cluster-span" ->
          if i + 1 >= Array.length Sys.argv then usage ();
          cluster_span := parse_int "--cluster-span" Sys.argv.(i + 1);
          loop (i + 2)
      | "--help" | "-h" -> usage ()
      | arg ->
          if !file = None then file := Some arg
            ...
            Printf.eprintf "[replay] unexpected argument: %s\n" arg;
            usage ());
          loop (i + 1)
  in
  loop 1;
...
let () =
  let file, n_edits, warmup, window_bytes, mode, cluster_span, csv =
    parse_cli ()
  in
  if window_bytes <= 0 then (
    prerr_endline "[replay] window must be > 0";
    exit 2);
  ...
  let edits =
    ...
    Array.init n_edits (fun _ ->
        ...
        | InsertOnly ->
            Insert
              ( Random.State.int rng (max 1 (String.length base)),
                Char.chr (97 + Random.State.int rng 26) )
        | DeleteOnly ->
            if String.length base > 0 then
              Delete (Random.State.int rng (String.length base))
            else Insert (0, 'a')
        | Balance ->
            if Random.State.bool rng then
              Insert
                ( Random.State.int rng (max 1 (String.length base)),
                  Char.chr (97 + Random.State.int rng 26) )
            else if String.length base > 0 then
              Delete (Random.State.int rng (String.length base))
            else Insert (0, 'a')
        | Cluster ->
            ...
            let pos =
              clamp 0
                (center
                + (Random.State.int rng ((2 * cluster_span) + 1) - cluster_span)
                )
                (max 0 (String.length base))
            in
            hotspot := pos;
            if Random.State.bool rng then
              Insert (pos, Char.chr (97 + Random.State.int rng 26))
            else if String.length base > 0 then
              Delete (clamp 0 pos (max 0 (String.length base - 1)))
            else Insert (pos, 'a'))
  in
  ...
  for _ = 1 to warmup do
    ...
    Pretouch.pre_touch_bytes
      (Bytes.unsafe_of_string slice)
      ~page:Config.page_bytes;
    ignore
      (Real_processor.run (Bytes.unsafe_of_string slice) (Arena.current arenas))
  done;
  ...
  Printf.printf
    "[replay] %s stats min=%.3f p50=%.3f p95=%.3f p99=%.3f p999=%.3f max=%.3f \
     (ms)\n\
     %!"
    mode_s min_ms p50 p95 p99 p999 max_ms;

  match csv with
  | Some p -> write_csv p [| min_ms; p50; p95; p99; p999; max_ms |]
  | None -> ()
