...
let usage () =
  prerr_endline
    "usage: edit_window_bench FILE ITERS [--warmup N] [--window BYTES] [--csv \
     PATH]";
    ...
    if i >= Array.length Sys.argv then ()
      ...
      | arg ->
          if !file = None then file := Some arg
            ...
            Printf.eprintf "[edit] unexpected argument: %s\n" arg;
            usage ());
          loop (i + 1)
  in
  loop 1;
  let win_list =
    if !windows = [] then [ default_window_bytes ] else List.rev !windows
  in
...
let () =
  ...
  if total <= 0 then (
    prerr_endline "[edit] iterations must be > 0";
    exit 2);
  ...
  List.iter
    (fun window_bytes ->
      if window_bytes <= 0 then (
        Printf.eprintf "[edit] window must be > 0 (got %d)\n" window_bytes;
        exit 2);
      if Bytes.length input < window_bytes then
        Printf.printf
          "[edit] SKIP (%dB): input smaller than window (%d < %d)\n%!"
          window_bytes (Bytes.length input) window_bytes
      else
        let pick_slice () =
          let max_offset = Bytes.length input - window_bytes in
          let offset =
            if max_offset = 0 then 0 else Random.State.int rng (max_offset + 1)
          in
        ...
        Bench_utils.Percentiles_strict.dump
          (Printf.sprintf "edit-window-%d" window_bytes)
          samples;
        Printf.printf
          "[edit] %dB stats min=%.3f p50=%.3f p95=%.3f p99=%.3f p999=%.3f \
           max=%.3f (ms)\n\
           %!"
        ...
        if n >= 1_000 then
          if p95 > 1.2 then (
            Printf.eprintf
              "[edit] FAIL (%dB): P95 = %.3f ms > 1.2 ms target\n%!"
              window_bytes p95;
            exit 3)
          else
