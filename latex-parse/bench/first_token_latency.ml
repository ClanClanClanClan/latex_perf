...
let usage () =
  prerr_endline "usage: first_token_latency FILE ITERS [--window BYTES] [--csv PATH]";
  exit 2
...
let parse_cli () =
  ...
  while !i < Array.length Sys.argv do
    ...
    | "--window" -> if !i+1 >= Array.length Sys.argv then usage (); window := parse_int "--window" Sys.argv.(!i+1); i := !i+2
...
let () =
  ...
  if iters <= 0 then (prerr_endline "[ft] iterations must be > 0"; exit 2);
  let input = read_file file in
  if Bytes.length input = 0 then (prerr_endline "[ft] empty input"; exit 2);
  let view = slice input window in
  let arenas = Arena.create ~cap:Config.arenas_tokens_cap in
  (try Simd_guard.require () with Failure msg -> prerr_endline ("[ft] FATAL: "^msg); exit 2);
  Gc_prep.init_gc ();
  ...
  Printf.printf "[ft] stats min=%.1f p50=%.1f p95=%.1f p99=%.1f p999=%.1f max=%.1f (us)\n%!"
    min_us p50 p95 p99 p999 max_us;

  (match csv with Some path -> write_csv path [|min_us;p50;p95;p99;p999;max_us|] | None -> ());

  if p95 > 350.0 then (Printf.eprintf "[ft] FAIL: P95 = %.1f us > 350 us target\n%!" p95; exit 3)
