(* Uncommitted measurement driver: classify each file (paths on stdin) into
   LP-Core / LP-Extended / LP-Foreign and print tier + triggering feature
   ids. *)
let read_file p =
  ...
  try
    while true do
      ...
      try
        ...
        let tier, feats =
          Latex_parse_lib.Language_profile.classify_source src
        in
        let ids =
          feats
        ...
        Printf.printf "%s\t%s\t%b\t%s\n"
          ...
          (let needle = "\\documentclass" in
           ...
           let rec go i =
             i + m <= n && (String.sub src i m = needle || go (i + 1))
           in
           go 0)
          path
