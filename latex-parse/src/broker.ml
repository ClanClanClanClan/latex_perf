...
let pick_hot p =
  ...
  let rec go i k =
    if k >= n then p.workers.(i mod n)
    else
      let w = p.workers.(i mod n) in
      if w.state = Hot && not w.inflight then (
        p.rr <- i + 1;
        w)
      else go (i + 1) (k + 1)
  in
...
let update_on_resp _p w ~alloc_mb10 ~major =
  ...
  if
    (w.alloc_mb >= ultra_aggressive_threshold
    || w.major >= Config.worker_major_cycles_budget - 1
    || (* Rotate 1 cycle earlier *)
    (w.alloc_mb >= 150.0 && w.major >= 1))
    && w.state = Hot (* Very early rotation *)
...
let next_req_id () =
  let id = !req_ctr in
  req_ctr := Int64.succ id;
  id

let drain_one_ready ~deadline_ns p =
  let fd1 = if p.workers.(0).inflight then int_of_fd p.workers.(0).fd else -1 in
  let fd2 = if p.workers.(1).inflight then int_of_fd p.workers.(1).fd else -1 in
  (if fd1 < 0 && fd2 < 0 then ()
   else
  ...
  if Int64.sub (Clock.now ()) deadline_ns > 0L then (
    ...
    Printf.eprintf
      "[broker] WARNING: Timeout recovery - cleared inflight flags\n%!")
...
let hedged_call p ~(input : bytes) ~(hedge_ms : int) : svc_result =
  ...
  | `Primary_ready -> (
      ...
      | Any_resp (rid, st, nt, iss, mb10, maj) when rid = req_id ->
          update_on_resp p primary ~alloc_mb10:mb10 ~major:maj;
          maybe_rotate p primary;
          {
            status = st;
            n_tokens = nt;
            issues_len = iss;
  ...
  | `Hedge_fire ->
      ...
      let rec race () =
        let _tf, ready =
          Hedge_timer.wait_two p.timer ~fd1:(int_of_fd primary.fd)
            ~fd2:(int_of_fd sec.fd)
        in
        if ready = int_of_fd primary.fd then
          ...
          | Any_resp (rid, st, nt, iss, mb10, maj) when rid = req_id ->
              update_on_resp p sec ~alloc_mb10:mb10 ~major:maj;
              maybe_rotate p sec;
              Ipc.write_cancel primary.fd ~req_id;
              primary.inflight <- false;
