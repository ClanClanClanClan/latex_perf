...
val analyze_files :
  base_dir:string -> ?tex_path:string -> source:string -> unit ->
...
(** [analyze_files ~base_dir ?tex_path ~source] extracts file references from
    ...
    entries), reads metadata from found files, and returns the analysis.
    If [tex_path] is provided, also looks for the compiled output PDF
    (same stem, .pdf extension) in [base_dir]. *)
