(* ══════════════════════════════════════════════════════════════════════
   ...
   Reads the font's character mapping table to check glyph coverage for CJK
   Unified Ideographs (U+4E00–U+9FFF). Supports format 4 (BMP) and format 12
   (full Unicode) subtables.
   ══════════════════════════════════════════════════════════════════════ *)
...
(* We check a sample of codepoints rather than all 20,992. If at least 80% of
   samples are covered, we declare coverage. *)
...
let format12_has_codepoint (buf : bytes) (off : int) (cp : int) : bool =
  ...
  if off + 16 > len then false
    ...
    while !i < n_groups && not !found do
      ...
      if g_off + 12 > len then i := n_groups (* stop *)
      else
  ...
  | Some ic ->
      ...
      if file_len < 12 then None
        ...
        for i = 0 to num_tables - 1 do
          ...
          if entry_off + 16 <= file_len then
            let tag = Bytes.sub_string buf entry_off 4 in
            if tag = "cmap" then cmap_off := read_u32_be buf (entry_off + 8)
        done;
        if !cmap_off < 0 || !cmap_off + 4 > file_len then None
          ...
          (* Find the best subtable: prefer format 12, then format 4 (platform 3
             = Windows, encoding 1 = Unicode BMP or 10 = full) *)
          let best_off = ref (-1) in
          let best_fmt = ref 0 in
          for i = 0 to num_subtables - 1 do
            let rec_off = cmap_base + 4 + (i * 8) in
            if rec_off + 8 <= file_len then
              ...
              if abs_off + 2 <= file_len then
                let fmt = read_u16_be buf abs_off in
                (* Prefer format 12 > format 4 *)
                if fmt = 12 && !best_fmt < 12 then (
                  best_off := abs_off;
                  best_fmt := 12)
                else if fmt = 4 && !best_fmt < 4 then (
                  best_off := abs_off;
                  best_fmt := 4)
          done;
