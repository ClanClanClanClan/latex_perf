...
CAMLprim value ocaml_ht_wait2(value vkq, value vfd1, value vfd2){
  ...
  int nset = 0;
  if (fd1 >= 0) { EV_SET(&kevset[nset++], fd1, EVFILT_READ, EV_ADD | EV_ENABLE | EV_CLEAR, 0, 0, NULL); }
  if (fd2 >= 0) { EV_SET(&kevset[nset++], fd2, EVFILT_READ, EV_ADD | EV_ENABLE | EV_CLEAR, 0, 0, NULL); }
  if (nset>0) {
