(* ══════════════════════════════════════════════════════════════════════
   ...
   Walks JPEG markers (FF xx) to extract dimensions, DPI, ICC profile info, and
   Adobe color transform. No external dependencies.
   ══════════════════════════════════════════════════════════════════════ *)
...
(** Extract DPI from EXIF APP1 segment data (after "Exif\0\0"). Returns (dpi_x,
    dpi_y) or (0.0, 0.0) if not found. *)
...
(** Parse ICC color space from 4 bytes at offset 16 in profile header. Returns
    [`RGB], [`CMYK], [`Gray], or [`Unknown]. *)
let parse_icc_color_space (buf : bytes) (profile_start : int) :
    [ `RGB | `CMYK | `Gray | `Unknown ] =
...
let read_jpeg_info (path : string) : jpeg_info option =
  ...
  | Some ic ->
      ...
      if file_len < 4 then None
        ...
        if
          Char.code (Bytes.get soi 0) <> 0xFF
          || Char.code (Bytes.get soi 1) <> 0xD8
        then None
        else
          let width = ref 0 in
          let height = ref 0 in
          ...
          while not !finished do
            try
              ...
              if b0 <> 0xFF then finished := true
              else
                match b1 with
                | 0xD9 (* EOI *) -> finished := true
                | 0xD8 (* SOI — skip *) -> ()
                | m when m >= 0xD0 && m <= 0xD7 ->
                ...
                | _ -> (
                    ...
                    if data_len < 0 then finished := true
                      ...
                      (* APP0 — JFIF *)
                      | 0xE0
                        when data_len >= 12 && starts_with data 0 "JFIF\000"
                        -> (
                      ...
                      (* APP1 — EXIF *)
                      | 0xE1
                        when data_len >= 14 && starts_with data 0 "Exif\000\000"
                        ->
                      ...
                      | 0xE2
                        when data_len >= 14
                             && starts_with data 0 "ICC_PROFILE\000" ->
                          (* data[12] = sequence number (1-based), data[13] =
                             total chunks *)
                          has_icc := true;
                          let seq_num = Char.code (Bytes.get data 12) in
                          (* Only read color space from the first chunk (seq=1).
                             ICC header byte 16 (color space) is at
                             data[14+16]=data[30], so we need data_len >= 34. *)
                          if seq_num = 1 && !icc_cs = `Unknown && data_len >= 34
                          then
                      ...
                      | _ -> ())
            with End_of_file | Sys_error _ -> finished := true
          done;
