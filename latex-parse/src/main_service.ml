open Unix
let () = Latex_parse_lib.Simd_guard.require ()  (* hard guard: refuse to start without SIMD unless L0_ALLOW_SCALAR=1 *)
...
let run () =
  ...
  (match Sys.getenv_opt "L0_PROM" with
   | Some "1" -> ignore (Thread.create Latex_parse_lib.Metrics_prometheus.serve ())
   | _ -> ());
  ...
  let handle_conn c =
    ...
    let rec loop () =
      ...
      let recv () = req in
      let last_result : Latex_parse_lib.Broker.svc_result option ref = ref None in
      let process (req:bytes) =
        try
          let r = Latex_parse_lib.Broker.hedged_call pool ~input:req ~hedge_ms:Latex_parse_lib.Config.hedge_timer_ms_default in
          last_result := Some r;
          `Ok r
        with _ ->
          last_result := None;
      ...
      let format = function
        | `Ok r ->
            ...
            put32 b 0 r.Latex_parse_lib.Broker.status; put32 b 4 r.Latex_parse_lib.Broker.n_tokens; put32 b 8 r.Latex_parse_lib.Broker.issues_len;
            Bytes.unsafe_set b 12 (match r.Latex_parse_lib.Broker.origin with `P->Char.unsafe_chr 1 | `H->Char.unsafe_chr 2); b
        | `Err ->
            let b = Bytes.create 13 in
      ...
      let (ms, st) = Latex_parse_lib.Service_bracket.measure_bytes_in_to_reply_ready ~recv ~process ~format ~send in
      Latex_parse_lib.Metrics_prometheus.observe_latency ms;
      (* get actual hedge telemetry from broker result *)
      let hedged, status =
        match !last_result with
        | Some r -> (r.Latex_parse_lib.Broker.hedge_fired, r.Latex_parse_lib.Broker.status)
        | None -> (false, 1)
      in
      if status <> 0 then Latex_parse_lib.Metrics_prometheus.on_error ();
      add_slowest ms (st.Latex_parse_lib.Service_bracket.t_in_start, st.t_in_done, st.t_proc_start, st.t_hedge_fire, st.t_first_reply, st.t_reply_ready, hedged, "");
      if Latex_parse_lib.Broker.(pool.requests mod 10_000 = 0) then begin
        Printf.eprintf "[hedge] req=%d fired=%d (%.3f%%) wins=%d (%.1f%%) rotations=%d\n%!"
          Latex_parse_lib.Broker.(pool.requests) Latex_parse_lib.Broker.(pool.hedge_fired)
          (100.0 *. float Latex_parse_lib.Broker.(pool.hedge_fired) /. float (max 1 Latex_parse_lib.Broker.(pool.requests)))
          Latex_parse_lib.Broker.(pool.hedge_wins)
          (100.0 *. float Latex_parse_lib.Broker.(pool.hedge_wins) /. float (max 1 Latex_parse_lib.Broker.(pool.hedge_fired)))
