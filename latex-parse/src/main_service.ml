...
let run () =
  ...
  let rec accept_loop () =
    try
      ...
      Unix.set_close_on_exec c;
      
      (* Handle one connection sequentially - original HELP_AI.md design *)
      (try
        let recv () =
        ...
        let process req =
          (* No mutex - test raw performance *)
          let result = Broker.hedged_call pool ~input:req ~hedge_ms:Config.hedge_timer_ms_default in
          result
        in
      ...
      with
      | Unix.Unix_error(Unix.EPIPE, _, _) -> () (* Client disconnected *)
      | Unix.Unix_error(Unix.ECONNRESET, _, _) -> () (* Connection reset *)
      | Failure _ -> () (* Client closed connection *)
      | _ -> () (* Log but continue *)
      );
      ...
      accept_loop ()
    with
    | Unix.Unix_error(Unix.EINTR, _, _) -> accept_loop () (* Interrupted, retry *)
    | e -> 
      Printf.eprintf "[svc] accept error: %s\n%!" (Printexc.to_string e);
      accept_loop ()
  in
  accept_loop ()
