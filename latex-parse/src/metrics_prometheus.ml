...
(* Minimal Prometheus exporter; enable with L0_PROM=1.
   Configuration:
   - L0_PROM_ADDR = "HOST:PORT" (default 127.0.0.1:9109)
   - L0_PROM_UDS  = "/path/to/socket" (serve over Unix domain socket)
   If both are set, UDS takes precedence. *)
...
let parse_addr s =
  ...
  | [h; p] ->
      let host =
        try inet_addr_of_string h with _ ->
...
let dump_metrics oc =
  ...
  fprintf oc "# HELP validators_rule_hits_total rule evaluations\n# TYPE validators_rule_hits_total counter\n";
...
let serve () =
  ...
  let serve_uds path =
    ...
    listen fd 64;
    eprintf "[prom] uds://%s (curl --unix-socket %s http://localhost/metrics)\n%!" path path;
    let rec loop () =
      let (c,_) = accept fd in
      ignore (Thread.create (fun cfd ->
        let buf = Bytes.create 1024 in ignore (Unix.read cfd buf 0 1024);
  ...
  try
    ...
    | _ ->
        let (h, p) = match tcp_opt with Some s -> parse_addr s | None -> (default_host, default_port) in
        serve_tcp ~host:h ~port:p
  with
  | Unix_error (e, _, _) -> eprintf "[prom] bind failed: %s (metrics disabled)\n%!" (error_message e)
