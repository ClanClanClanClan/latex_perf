(* ══════════════════════════════════════════════════════════════════════
   ...
   Architecture: 1. Xref parser: scan from EOF for startxref, read xref table 2.
   Token/object parser: dicts, arrays, strings, names, numbers, refs 3. Catalog
   navigator: Root → StructTreeRoot, MarkInfo, Lang, etc. 4. Page tree walker:
   Pages → Resources → Font, ColorSpace, Annots 5. Font inspector:
   FontDescriptor → FontFile presence 6. Annotation inspector: Link → Contents
   7. Structure tree walker: K → Figure → Alt 8. ColorSpace inspector:
   Separation, DeviceN 9. Stream inspector: Filter presence
...
type parse_state = { buf : bytes; mutable pos : int; len : int }

let ps_of_bytes b = { buf = b; pos = 0; len = Bytes.length b }

let _ps_peek ps =
  if ps.pos >= ps.len then None else Some (Bytes.get ps.buf ps.pos)
...
let ps_skip_whitespace ps =
  ...
  (* Also skip comments *)
  if ps.pos < ps.len && Bytes.get ps.buf ps.pos = '%' then (
...
let parse_number ps =
  ...
  if
    ps.pos < ps.len
    && (Bytes.get ps.buf ps.pos = '-' || Bytes.get ps.buf ps.pos = '+')
  then ps.pos <- ps.pos + 1;
  while
    ps.pos < ps.len
    &&
    let c = Bytes.get ps.buf ps.pos in
  ...
  else try Some (PdfInt (int_of_string s)) with Failure _ -> None
...
let parse_literal_string ps =
  if ps.pos >= ps.len || Bytes.get ps.buf ps.pos <> '(' then None
    ...
    while ps.pos < ps.len && !depth > 0 do
      ...
      | '\\' when ps.pos < ps.len -> (
          ...
          | c when is_digit c ->
              ...
              for _ = 1 to 2 do
                if
                  ps.pos < ps.len
                  && is_digit (Bytes.get ps.buf ps.pos)
                  && Char.code (Bytes.get ps.buf ps.pos) <= Char.code '7'
                then (
                  oct :=
                    (!oct * 8)
                    + (Char.code (Bytes.get ps.buf ps.pos) - Char.code '0');
                  ps.pos <- ps.pos + 1)
              done;
      ...
      | ')' ->
          decr depth;
          if !depth > 0 then Buffer.add_char b c
...
let rec parse_value ps =
  ...
  if ps.pos >= ps.len then None
    ...
    | '/' -> (
        (* Name *)
        match parse_name ps with Some n -> Some (PdfName n) | None -> None)
    | '(' ->
        (* Literal string *)
    ...
    | '[' ->
        ...
        while not !stop do
          ...
          else if Bytes.get ps.buf ps.pos = ']' then (
            ps.pos <- ps.pos + 1;
            stop := true)
          else
  ...
  if ps.pos + 1 >= ps.len then None
    ...
    while not !stop do
      ...
      if ps.pos + 1 >= ps.len then stop := true
        ...
        Bytes.get ps.buf ps.pos = '>' && Bytes.get ps.buf (ps.pos + 1) = '>'
      then (
        ps.pos <- ps.pos + 2;
        stop := true)
      else
...
let find_startxref (buf : bytes) : int option =
  ...
  for i = search_start to len - klen - 1 do
    if !result = None && Bytes.sub_string buf i klen = keyword then (
      (* Skip whitespace after keyword *)
      let j = ref (i + klen) in
      while !j < len && is_whitespace (Bytes.get buf !j) do
        incr j
      done;
      (* Read number *)
      let start = !j in
      while !j < len && is_digit (Bytes.get buf !j) do
        incr j
      done;
      if !j > start then
        try
          result :=
            Some (int_of_string (Bytes.sub_string buf start (!j - start)))
        with Failure _ -> ())
  done;
  !result

(** Parse a standard xref table starting at [pos]. Returns (object_offsets,
    trailer_dict). *)
let parse_xref_table (buf : bytes) (pos : int) :
  ...
  if not (ps_match_keyword ps "xref") then None
            ...
            | Some (PdfInt count) ->
                for i = 0 to count - 1 do
                  ...
                  if ps.pos + 20 <= ps.len then (
                    ...
                    while
                      ps.pos < ps.len
                      && (Bytes.get ps.buf ps.pos = '\r'
                         || Bytes.get ps.buf ps.pos = '\n')
                    do
                      ps.pos <- ps.pos + 1
                    done;
                    if flag = 'n' then
                      try
                        ...
                        if off > 0 then
                          Hashtbl.replace offsets (first_obj + i) off
                      with Failure _ -> ())
                  else ps.pos <- ps.len
                done
            | _ -> finished := true)
    ...
    (* Parse trailer dict *)
    ps_skip_whitespace ps;
    if ps_match_keyword ps "trailer" then (
      ps_skip_whitespace ps;
...
(** Read an indirect object at the given byte offset. Format: "N G obj ...
    endobj" *)
let read_object_at (buf : bytes) (offset : int) : pdf_value option =
  ...
  | Some (PdfInt _obj_num) -> (
      ...
      | Some (PdfInt _gen) ->
          ps_skip_whitespace ps;
          if ps_match_keyword ps "obj" then (
            ps_skip_whitespace ps;
        ...
        if not (Bytes.sub_string buf 0 (min 5 file_len) = "%PDF-") then None
          ...
          | Some xref_pos -> (
              ...
              | Some (offsets, trailer) -> (
                  ...
                  match dict_get "Encrypt" trailer with
                  | Some _ -> None
                  | None ->
                      (* Read all objects *)
                      ...
                      (match dict_get "Prev" trailer with
                      | Some (PdfInt prev_xref) when prev_xref > 0 -> (
                          match parse_xref_table buf prev_xref with
                          | Some (prev_offsets, _) ->
                              Hashtbl.iter
                                (fun obj_num offset ->
                                  if
                                    (not (Hashtbl.mem objects obj_num))
                                    && offset < file_len
                                  then
                                    match read_object_at buf offset with
                                    | Some v ->
                                        Hashtbl.replace objects obj_num v
                                    | None -> ())
                                prev_offsets
                          | None -> ())
                      | _ -> ());
...
let resolve (doc : pdf_document) (v : pdf_value) : pdf_value =
  let rec go depth v =
    if depth > 20 then PdfNull (* guard against cycles *)
      ...
      match v with
      | PdfRef (n, _) -> (
          match Hashtbl.find_opt doc.objects n with
          | Some v' -> go (depth + 1) v'
          | None -> PdfNull)
      | _ -> v
  in
...
let resolve_array doc v =
  match resolve doc v with PdfArray items -> Some items | _ -> None

let _resolve_name doc v =
  match resolve doc v with PdfName n -> Some n | _ -> None

let resolve_string doc v =
  match resolve doc v with PdfString s -> Some s | _ -> None

let _resolve_int doc v =
  match resolve doc v with PdfInt n -> Some n | _ -> None
...
let collect_pages (doc : pdf_document) : (string * pdf_value) list list =
  ...
  let rec walk v depth =
    if depth > 50 then () (* guard *)
      ...
      | Some entries -> (
          match dict_get "Type" entries with
          | Some (PdfName "Pages") -> (
              match dict_get "Kids" entries with
              | Some kids -> (
                  match resolve_array doc kids with
                  | Some items ->
                      List.iter (fun kid -> walk kid (depth + 1)) items
                  | None -> ())
              | None -> ())
          | Some (PdfName "Page") | _ -> pages := entries :: !pages)
      | None -> ()
  in
  (match get_catalog doc with
  | Some cat -> (
      match dict_get "Pages" cat with
...
(* ══════════════════════════════════════════════════════════════════════ §8
   Structure Tree Inspector
   ══════════════════════════════════════════════════════════════════════ *)

(** Count /Figure structure elements without /Alt text. *)
let count_figures_without_alt (doc : pdf_document)
    (struct_tree : (string * pdf_value) list) : int =
  let count = ref 0 in
  let rec walk v depth =
    if depth > 100 then ()
    else
      ...
      | None -> ()
  in
  (match dict_get "K" struct_tree with
  | Some (PdfArray items) -> List.iter (fun item -> walk item 0) items
...
let count_links_without_contents (doc : pdf_document)
  ...
  List.iter
    (fun page_entries ->
      ...
      | Some annots_val -> (
          match resolve_array doc annots_val with
          | Some annots ->
              List.iter
                (fun annot ->
...
let extract_fonts (doc : pdf_document) (pages : (string * pdf_value) list list)
  ...
  List.iter
    (fun page_entries ->
      ...
      | Some res_val -> (
          match resolve_dict doc res_val with
          | Some res_entries -> (
              match dict_get "Font" res_entries with
              | Some font_dict_val -> (
                  match resolve_dict doc font_dict_val with
                  | Some font_entries ->
                      List.iter
                        (fun (_key, font_ref) ->
                          ...
                          | Some font ->
                              let base_name =
                                ...
                                | _ -> "Unknown"
                              in
                              if not (Hashtbl.mem fonts base_name) then
                                let embedded, subsetted =
                                  match dict_get "FontDescriptor" font with
                                  | Some fd_val -> (
                                      match resolve_dict doc fd_val with
                                      | Some fd ->
                                          let has_file =
                                          ...
                                          let is_subsetted =
                                            ...
                                            &&
                                            let prefix =
                                              String.sub base_name 0 6
                                            in
                                            String.uppercase_ascii prefix
                                            = prefix
                                          in
                                  ...
                                  | None -> (false, false)
                                in
                                Hashtbl.replace fonts base_name
                                  (embedded, subsetted)
                          | None -> ())
                        font_entries
                  | None -> ())
  ...
  List.iter
    (fun page_entries ->
      ...
      | Some res_val -> (
          match resolve_dict doc res_val with
          | Some res_entries -> (
              match dict_get "ColorSpace" res_entries with
              | Some cs_val -> (
                  match resolve_dict doc cs_val with
                  | Some cs_entries ->
                      List.iter
                        (fun (_key, cs_ref) ->
                  ...
                  | None -> (
                      ...
                      | PdfArray (PdfName "Separation" :: _) -> has_spot := true
                      | PdfArray (PdfName "DeviceN" :: _) ->
                          has_spot := true;
                          has_spot_vector := true
                      | _ -> ()))
...
let check_streams_compressed (doc : pdf_document) : bool =
  ...
  Hashtbl.iter
    (fun _obj_num v ->
      ...
      | PdfStream (entries, _) -> (
          has_streams := true;
          match dict_get "Filter" entries with
          | Some _ -> ()
...
let count_page_labels (doc : pdf_document) (cat : (string * pdf_value) list) :
    int =
  match dict_get "PageLabels" cat with
  | Some pl_ref -> (
      match resolve_dict doc pl_ref with
      | Some pl_entries -> (
          match dict_get "Nums" pl_entries with
          | Some nums_val -> (
              match resolve_array doc nums_val with
              | Some items ->
                  List.length items / 2 (* pairs of page_num, label_dict *)
              | None -> 0)
          | None -> 0)
      | None -> 0)
  | None -> 0
  ...
  | Some doc -> (
      ...
      | Some cat ->
          ...
          let has_mark_info =
            ...
            | Some mi_ref -> (
                match resolve_dict doc mi_ref with
                | Some entries -> (
                    match dict_get "Marked" entries with
          ...
          let figures_without_alt =
            match dict_get "StructTreeRoot" cat with
            | Some st_ref -> (
                match resolve_dict doc st_ref with
          ...
          let links_without_contents = count_links_without_contents doc pages in
