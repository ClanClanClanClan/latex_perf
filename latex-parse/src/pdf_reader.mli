(* ══════════════════════════════════════════════════════════════════════
   ...
   Reads PDF cross-reference tables, resolves indirect object references, and
   navigates catalog/pages/fonts/annotations/structure tree to extract metadata
   needed by L3 validator rules.

   Handles standard xref tables and cross-reference streams (PDF 1.5+).
   Encrypted PDFs and malformed files return [None] gracefully. No zlib
   decompression — inspects dictionary keys only.
   ══════════════════════════════════════════════════════════════════════ *)
...
type pdf_info = {
  ...
  lang : string option;
  fonts : (string * bool * bool) list;  (** (name, is_embedded, is_subsetted) *)
...
(** [read_pdf_info path] extracts high-level metadata from a PDF file. Returns
    [None] on I/O error or unparseable PDF. *)
