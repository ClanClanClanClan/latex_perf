(* ══════════════════════════════════════════════════════════════════════
   ...
   Reads PNG chunk headers and small metadata chunks (IHDR, pHYs, tRNS, iCCP,
   sRGB, PLTE) without decompressing IDAT. No external dependencies — pure OCaml
   binary parsing.
   ══════════════════════════════════════════════════════════════════════ *)
...
let read_u8 (buf : bytes) (off : int) : int = Char.code (Bytes.get buf off)
             ...
             while not !finished do
               try
                 ...
                 | "iCCP" when chunk_len >= 2 ->
                     ...
                     if remaining > 0 then seek_in ic (pos_in ic + remaining);
