...
let send_response client_sock status headers body =
  ...
  let response = response ^ sprintf "Content-Length: %d\r\n\r\n" (String.length body) in
  let response = response ^ body in
  ignore (Unix.write client_sock (Bytes.of_string response) 0 (String.length response))
...
let parse_http_request request =
  ...
  | first_line :: rest ->
    ...
    let method_opt = match parts with
      | meth :: _ -> Some meth
      | _ -> None
    in
    let path_opt = match parts with
      | _ :: path :: _ -> Some path
      | _ -> None
    in
    ...
    let rec find_body lines in_header acc_header =
      ...
      | line :: rest ->
        ...
        if in_header && line = "" then
          ...
          find_body rest true (line :: acc_header)
        else
          find_body rest false acc_header
    in
...
let call_simd_service latex_content =
  ...
  | Some service_sock ->
    try
      ...
      if n <> 4 then (
        Unix.close service_sock; Error "Invalid response from service (short header)"
      ) else (
        let be32 b off =
        ...
        if total_read <> resp_len then Error "Incomplete response from service"
...
let json_ok ~expanded ~status ~origin ~n_tokens ~issues_len ~req_bytes ~validators ~v_duration_ms ~v_timings =
  let svc = `Assoc [ ("name", `String service_name); ("version", `String service_version); ("ts", `Int (now_ts ())) ] in
  let metrics = `Assoc [ ("n_tokens", `Int n_tokens); ("issues_len", `Int issues_len); ("request_bytes", `Int req_bytes) ] in
  let v_applied = List.length validators in
  let v_errors = List.fold_left (fun acc (_id, sev, _msg, cnt) -> match sev with `String "error" -> acc + cnt | _ -> acc) 0 validators in
  let v_results =
    `List (List.map (fun (id, sev, msg, cnt) -> `Assoc [
      ("id", `String id);
      ("severity", sev);
      ("message", `String msg);
      ("count", `Int cnt)
  ...
  let validators =
    ...
    | Some ("1"|"true"|"on") ->
        (match validators with `Assoc kv -> `Assoc (("duration_ms", `Float v_duration_ms) :: kv) | _ -> validators)
    | _ -> validators
  in
  let validators =
    match Sys.getenv_opt "L0_INCLUDE_VALIDATORS_TIMINGS" with
    | Some ("1"|"true"|"on") ->
        let arr = `List (List.map (fun (id, ms) -> `Assoc [("id", `String id); ("duration_ms", `Float ms)]) v_timings) in
        (match validators with `Assoc kv -> `Assoc (("timings", arr) :: kv) | _ -> validators)
    | _ -> validators
  in
  let base = [
    ("ok", `Bool (status = 0));
  ...
  let base = match expanded with None -> base | Some s -> ("expanded", `String s) :: base in
  Y.to_string @@ `Assoc base

let handle_tokenize body =
  (* Parse JSON request with Yojson: expect {"latex": "...", "expand": bool?, "catalogue": obj?} *)
  let extract_latex_content body =
    try
      match Y.from_string body with
      | `Assoc props ->
          let latex = (match List.assoc_opt "latex" props with Some (`String s) -> Some s | _ -> None) in
          let expand = (match List.assoc_opt "expand" props with Some (`Bool b) -> b | _ -> false) in
          let cfg = (match List.assoc_opt "catalogue" props with Some j -> Rest_simple_expander.of_json j | _ -> Rest_simple_expander.default) in
          (match latex with
           | Some s -> let s' = if expand then Rest_simple_expander.expand_fix cfg s else s in Some (s', expand)
           | None -> None)
      | _ -> None
    with _ -> None
  in
  ...
  | None ->
    let error_json = json_error ~code:400 "Invalid request format. Expected {\"latex\": \"...\"}" in
  ...
  | Some (latex_content, did_expand) ->
    ...
    | Ok resp ->
      ...
      begin match Latex_parse_lib.Service_payload.parse_payload resp with
      | Error e ->
          let json = json_error ~code:503 e in
          http_service_unavailable, json
      | Ok p ->
          let origin = match p.Latex_parse_lib.Service_payload.origin with
          ...
          let vres, vdur, vtim =
            ...
            let xs, dur, timings = Validators.run_all_with_timings latex_content in
            (List.map (fun (r: Latex_parse_lib.Validators.result) ->
          ...
          in
          let json = json_ok ~expanded:(if did_expand then Some latex_content else None)
            ~status:p.status ~origin ~n_tokens:p.n_tokens ~issues_len:p.issues_len ~req_bytes:(String.length latex_content) ~validators:vres ~v_duration_ms:vdur ~v_timings:vtim in
          (if p.status = 0 then http_ok else http_service_unavailable), json
      end
...
let handle_health () =
  ...
  | Ok resp ->
      ...
      | Ok p when p.Latex_parse_lib.Service_payload.status = 0 ->
          let json = Y.to_string @@ `Assoc [
            ("status", `String "healthy");
            ("n_tokens", `Int p.n_tokens)
          ] in
          http_ok, json
      | Ok p ->
          let json = json_error ~code:503 (Printf.sprintf "status=%d" p.status) in
          http_service_unavailable, json
      | Error e ->
          let json = json_error ~code:503 e in
          http_service_unavailable, json
...
let handle_metrics () =
  let metrics = [
    "# HELP latex_tokenizer_requests_total Total number of tokenization requests";
  ...
  let is_up = match connect_to_service () with
    | None -> "0"
    | Some sock -> Unix.close sock; "1"
  in

  let metrics_text = String.concat "\n" metrics ^ sprintf "latex_tokenizer_up %s\n" is_up in
...
let handle_client client_sock _client_addr =
  ...
  try
    ...
    if n > 0 then
      ...
      let status, response_body = match method_opt, path_opt with
        | Some "OPTIONS", _ ->
        ...
        | Some "POST", Some "/tokenize" ->
          if String.length body > max_request_size then
            ...
            handle_tokenize body

        | Some "POST", Some path when String.length path >= 7 && String.sub path 0 7 = "/expand" ->
          (try
             match Y.from_string body with
             | `Assoc props ->
                  ...
                  | Some (`String s) ->
                      ...
                      let layer =
                        if String.contains path '?' then (
                          try
                            let q = String.sub path (String.index path '?' + 1) (String.length path - (String.index path '?' + 1)) in
                              ...
                              let rec find_layer = function
                                ...
                                | kv::rest ->
                                    (match String.split_on_char '=' kv with
                                     | [k; v] when k = "layer" && v = "l1" -> Validators.L1
                                     | _ -> find_layer rest)
                              in find_layer parts
                            else Validators.L0
                      ...
                      in
                      (* Tokenize expanded text and build per-request command spans for validator context *)
                      ...
                      let cmd_spans = List.fold_left (fun acc (t:T.tok) -> match t.kind with
                        | T.Command ->
                            let i' = t.s + 1 in
                            let k = ref i' in
                            while !k < n && ((let ch = String.unsafe_get expanded !k in
                                              (ch>='a'&&ch<='z')||(ch>='A'&&ch<='Z'))) do incr k done;
                            if !k > i' then ({ Latex_parse_lib.Validators_context.name = String.sub expanded i' (!k - i'); s = t.s; e = t.e } : Latex_parse_lib.Validators_context.post_command) :: acc else acc
                        | _ -> acc
                      ) [] toks |> List.rev in
                      Latex_parse_lib.Validators_context.set_post_commands cmd_spans;
                      let xs, vdur, vtim = Validators.run_all_with_timings_for_layer expanded layer in
                      let tuples = List.map (fun (r: Latex_parse_lib.Validators.result) ->
                        let open Latex_parse_lib.Validators in
                        let { id; severity; message; count } = r in
                        (id, `String (severity_to_string severity), message, count)
                      ) xs in
                      (* Build JSON; include post-expansion tokens if debug flag is set *)
                      let include_post = match Sys.getenv_opt "L0_DEBUG_L1" with Some ("1"|"true"|"on") -> true | _ -> false in
                      let base_json = json_ok ~expanded:(Some expanded)
                        ~status:0 ~origin:"primary" ~n_tokens:0 ~issues_len:0 ~req_bytes:(String.length s) ~validators:tuples ~v_duration_ms:vdur ~v_timings:vtim in
                      let json = if include_post then (
                        let kind_to_string = function
                          | T.Word -> "word" | T.Space -> "space" | T.Newline -> "newline" | T.Command -> "command"
                          | T.Bracket_open -> "bracket_open" | T.Bracket_close -> "bracket_close" | T.Quote -> "quote" | T.Symbol -> "symbol"
                        in
                        let arr = `List (List.map (fun (t:T.tok) -> `Assoc [
                          ("kind", `String (kind_to_string t.kind));
                          ("s", `Int t.s); ("e", `Int t.e);
                          ("ch", match t.ch with None -> `Null | Some c -> `String (String.make 1 c))
                        ]) toks) in
                        let cmds = `List (List.map (fun (pc:Latex_parse_lib.Validators_context.post_command) -> `String pc.name) cmd_spans) in
                        let post_summary =
                          let ecmds = `List (List.map (fun (pc:Latex_parse_lib.Validators_context.post_command) -> `Assoc [
                            ("name", `String pc.name); ("s", `Int pc.s); ("e", `Int pc.e)
                          ]) cmd_spans) in
                          `Assoc [("commands", ecmds)]
                        in
                        match Y.from_string base_json with
                        | `Assoc kv -> Y.to_string (`Assoc (("post_tokens", arr) :: ("post_commands", cmds) :: ("post_summary", post_summary) :: kv))
                        | _ -> base_json
                      ) else base_json in
                      http_ok, json
                  | _ -> http_bad_request, json_error ~code:400 "Missing 'latex' field")
             | _ -> http_bad_request, json_error ~code:400 "Invalid JSON"
           with _ -> http_bad_request, json_error ~code:400 "Invalid request")
        ...
        | Some "POST", Some "/tokens" ->
          (match Sys.getenv_opt "L0_DEBUG_TOKENS" with
           | Some ("1"|"true"|"on") ->
               (try
                  match Y.from_string body with
                  | `Assoc props -> (match List.assoc_opt "latex" props with
                      | Some (`String s) ->
                          let module T = Latex_parse_lib.Tokenizer_lite in
                          ...
                          http_ok, Y.to_string (`Assoc [("count", `Int (List.length toks)); ("tokens", arr)])
                      | _ -> http_bad_request, json_error ~code:400 "Missing 'latex' field")
                  | _ -> http_bad_request, json_error ~code:400 "Invalid JSON"
                with _ -> http_bad_request, json_error ~code:400 "Invalid request")
           | _ -> http_bad_request, json_error ~code:403 "token debugging disabled")

        | Some "GET", Some "/ready" ->
          (* Cheap probe: only tests socket readiness, not full processing *)
          (match connect_to_service () with
           | None -> http_service_unavailable, json_error ~code:503 "service not ready"
           | Some sock -> Unix.close sock; http_ok, Y.to_string @@ `Assoc [ ("ready", `Bool true) ])
...
let start_server port =
  ...
  while true do
    try
      ...
      ignore (Thread.create (fun () -> handle_client client_sock client_addr) ())
    with
