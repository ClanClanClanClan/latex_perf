...
// External attestation functions from simd_tokenizer_fixed.c
extern uint64_t simd_get_neon_blocks_processed(void);
extern uint64_t simd_get_scalar_bytes_processed(void);
extern uint64_t simd_get_total_tokenize_calls(void);
extern void simd_reset_attestation_counters(void);

/*
 * SIMD Attestation OCaml Stubs
 */
CAMLprim value ocaml_simd_get_neon_blocks_processed(value unit) {
    return Val_int((int)simd_get_neon_blocks_processed());
}

CAMLprim value ocaml_simd_get_scalar_bytes_processed(value unit) {
    return Val_int((int)simd_get_scalar_bytes_processed());
}

CAMLprim value ocaml_simd_get_total_tokenize_calls(value unit) {
    return Val_int((int)simd_get_total_tokenize_calls());
}

CAMLprim value ocaml_simd_reset_attestation_counters(value unit) {
    simd_reset_attestation_counters();
    return Val_unit;
}