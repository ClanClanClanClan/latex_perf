(* ══════════════════════════════════════════════════════════════════════
   ...
   Generates minimal valid PNG, JPEG, PDF, and TTF files with configurable
   properties for testing readers and validators.
   ══════════════════════════════════════════════════════════════════════ *)
...
let crc32 (data : bytes) (off : int) (len : int) : int32 =
  ...
  for i = off to off + len - 1 do
    let idx =
      Int32.to_int
        (Int32.logand
           (Int32.logxor !c (Int32.of_int (Char.code (Bytes.get data i))))
           0xFFl)
    in
...
let make_jpeg (opts : jpeg_opts) : bytes =
  ...
  (if opts.j_has_icc then
     let total_chunks = max 1 opts.j_icc_chunks in
     for chunk = 1 to total_chunks do
       (* Chunk 1 carries the ICC header (128 bytes min); subsequent chunks
          carry dummy continuation data. *)
       let icc_data =
         if chunk = 1 then (
           let hdr = Bytes.make 128 '\000' in
           (* Color space at ICC header offset 16 *)
           let cs = opts.j_icc_color_space in
           if String.length cs >= 4 then Bytes.blit_string cs 0 hdr 16 4;
           hdr)
         else
       ...
       Bytes.set app2 16 (Char.chr chunk);
       (* sequence number *)
       Bytes.set app2 17 (Char.chr total_chunks);
       (* total chunks *)
  ...
  (* APP14 Adobe *)
  if opts.j_adobe_transform >= 0 then (
  ...
  for i = 0 to opts.j_components - 1 do
    let off = 10 + (i * 3) in
    if off + 2 < Bytes.length sof then (
      Bytes.set sof off (Char.chr (i + 1));
      (* component id *)
      Bytes.set sof (off + 1) '\x11';
      (* sampling factor *)
      Bytes.set sof (off + 2) (Char.chr 0)
      (* quant table *))
  done;
  add sof;
...
let make_pdf (opts : pdf_opts) : bytes =
  ...
  in
  (match struct_tree_num with
  | Some n ->
      cat_parts := Printf.sprintf "/StructTreeRoot %d 0 R" n :: !cat_parts
  | None -> ());

  if opts.has_mark_info then
    cat_parts := "/MarkInfo << /Marked true >>" :: !cat_parts;

  (match opts.has_lang with
  ...
  let cat_str =
    Printf.sprintf "%d 0 obj\n%s\nendobj\n" catalog_num cat_content
  in
  (* Replace placeholder *)
  ...
  (match opts.has_lang with
  | Some lang -> Buffer.add_string cat_buf (Printf.sprintf " /Lang (%s)" lang)
  | None -> ());
  ...
  if opts.fonts <> [] then (
    ...
    List.iteri
      (fun i (name, embedded, subsetted) ->
        let font_name = if subsetted then "ABCDEF+" ^ name else name in
        let fd_content =
          if embedded then
            Printf.sprintf
              "<< /Type /FontDescriptor /FontName /%s /FontFile2 << /Length 0 \
               >> >>"
              font_name
          else
        ...
        let font_content =
          Printf.sprintf
            "<< /Type /Font /BaseFont /%s /FontDescriptor %d 0 R >>" font_name
            fd_num
        in
  ...
  if opts.has_spot_colour then (
    Buffer.add_string res_buf "/ColorSpace << ";
    Buffer.add_string res_buf
      "/CS1 [/Separation /PANTONE#20Red /DeviceCMYK << >>] ";
    if opts.has_spot_colour_vector then
      Buffer.add_string res_buf
        "/CS2 [/DeviceN [/Cyan /Magenta] /DeviceCMYK << >>] ";
    Buffer.add_string res_buf ">> ");
  ...
  (* Build annotations *)
  let annots_str =
    if opts.links_without_contents > 0 then (
      let annot_nums = ref [] in
      for _ = 1 to opts.links_without_contents do
        let n =
          alloc_extra "<< /Type /Annot /Subtype /Link /Rect [0 0 100 100] >>"
        in
        annot_nums := n :: !annot_nums
      done;
      let refs =
        List.map (fun n -> Printf.sprintf "%d 0 R" n) (List.rev !annot_nums)
      in
  ...
  List.iter
    (fun (n, content) ->
      let off = Buffer.length buf in
      if (not opts.streams_compressed) && content = "" then
  ...
  Buffer.add_string buf
    (Printf.sprintf "<< /Size %d /Root 1 0 R >>\n" (max_obj + 1));
...
let make_ttf ~(has_cjk : bool) : bytes =
  ...
  put_u32_be header 0 0x00010000;
  (* sfnt version *)
  put_u16_be header 4 1;
  (* numTables *)
  put_u16_be header 6 16;
  (* searchRange *)
  put_u16_be header 8 0;
  (* entrySelector *)
  put_u16_be header 10 16;
  (* rangeShift *)
  Buffer.add_bytes buf header;

  (* Table record for cmap *)
  let cmap_offset = 12 + 16 in
  (* after header + 1 table record *)
  let table_rec = Bytes.create 16 in
  Bytes.blit_string "cmap" 0 table_rec 0 4;
  put_u32_be table_rec 4 0;
  (* checksum *)
  put_u32_be table_rec 8 cmap_offset;
  ...
  put_u16_be cmap_header 0 0;
  (* version *)
  put_u16_be cmap_header 2 1;
  (* numTables *)
  Buffer.add_bytes cmap_buf cmap_header;

  (* Encoding record: platform 3, encoding 10 (full Unicode) *)
  let enc_rec = Bytes.create 8 in
  put_u16_be enc_rec 0 3;
  (* platformID *)
  put_u16_be enc_rec 2 10;
  (* encodingID *)
  put_u32_be enc_rec 4 12;
  (* offset to subtable from cmap start *)
  ...
  put_u16_be subtable 0 12;
  (* format *)
  put_u16_be subtable 2 0;
  (* reserved *)
  put_u32_be subtable 4 subtable_len;
  (* length *)
  put_u32_be subtable 8 0;
  (* language *)
  put_u32_be subtable 12 n_groups;
  (* numGroups *)
  List.iteri
    (fun i (start_code, end_code) ->
      let off = 16 + (i * 12) in
      put_u32_be subtable off start_code;
      put_u32_be subtable (off + 4) end_code;
      put_u32_be subtable (off + 8) (start_code + 1)
      (* startGlyphID *))
    groups;
  Buffer.add_bytes cmap_buf subtable;

  let cmap_data = Buffer.contents cmap_buf in
  put_u32_be table_rec 12 (String.length cmap_data);
  (* length *)
...
let make_ttf_format4 ~(has_cjk : bool) : bytes =
  (* Build a minimal TTF with a format 4 (BMP) cmap subtable. Platform 3,
     encoding 1 = Windows Unicode BMP. *)
  let buf = Buffer.create 512 in
  ...
  let segments =
    if has_cjk then
      ...
      [
        (0x0020, 0x007E, 1);
        (* startGlyphID = 1 for ASCII *)
        (0x4E00, 0x9FFF, 0x60);
        (* startGlyphID for CJK *)
        (0xFFFF, 0xFFFF, 1);
      ] (* required sentinel *)
    else (* ASCII only + sentinel *)
      [ (0x0020, 0x007E, 1); (0xFFFF, 0xFFFF, 1) ]
  in
  ...
  (* Header: 14 bytes *)
  let subtable_len = 14 + (seg_count * 2 * 4) + 2 in
  (* 4 arrays + reservedPad *)
  let fmt4_header = Bytes.create 14 in
  put_u16_be fmt4_header 0 4;
  (* format *)
  put_u16_be fmt4_header 2 subtable_len;
  (* length *)
  put_u16_be fmt4_header 4 0;
  (* language *)
  put_u16_be fmt4_header 6 seg_count_x2;
  (* segCountX2 *)
  put_u16_be fmt4_header 8 search_range;
  (* searchRange *)
  put_u16_be fmt4_header 10 entry_selector;
  (* entrySelector *)
  put_u16_be fmt4_header 12 range_shift;
  (* rangeShift *)
  ...
  List.iteri
    (fun i (_start_code, end_code, _glyph) ->
  ...
  List.iteri
    (fun i (start_code, _end_code, _glyph) ->
  ...
  List.iteri
    (fun i (start_code, _end_code, start_glyph_id) ->
...
(** Generate a minimal valid JPEG with an EXIF APP1 marker carrying XResolution,
    YResolution, and ResolutionUnit IFD entries.

    [~dpi]: density value written into both XResolution and YResolution as a
    rational (dpi/1). [~big_endian]: if true the TIFF header uses "MM"
    (big-endian); otherwise "II" (little-endian). [~res_unit]: 2 = inches
    (default), 3 = centimetres. [~include_jfif]: if true an APP0 JFIF marker
    with 72 DPI is also emitted (defaults to true). *)
let make_jpeg_exif ?(res_unit = 2) ?(include_jfif = true) ~(dpi : int)
  ...
  if include_jfif then (
    ...
    Bytes.set app0 11 '\001';
    (* units = DPI *)
    put_u16_be app0 12 72;
    (* X density *)
    put_u16_be app0 14 72;
    (* Y density *)
    Bytes.set app0 16 '\000';
    Bytes.set app0 17 '\000';
    add app0);
  ...
  (* ── APP1 EXIF segment ─────────────────────────────────────────── *)
  (* Layout inside the APP1 data (after the 2-byte marker FF E1): [0..5]
     "Exif\0\0" --- TIFF header starts at offset 6 (= tiff_start) --- [6..7]
     byte order: "II" or "MM" [8..9] magic 0x002A [10..13] IFD0 offset from
     tiff_start (= 8, right after header) --- IFD0 at tiff_start + 8 = offset 14
     --- [14..15] entry count = 3 [16..27] entry 0: XResolution tag=0x011A
     type=5 count=1 valoff [28..39] entry 1: YResolution tag=0x011B type=5
     count=1 valoff [40..51] entry 2: ResolutionUnit tag=0x0128 type=3 count=1
     val [52..55] next IFD offset = 0 --- rational values --- [56..63]
     XResolution rational (num 4 + den 4) [64..71] YResolution rational (num 4 +
     den 4) Total EXIF data length = 72 bytes. *)
  let exif_data_len = 72 in
  (* APP1 segment: marker(2) + length(2) + data *)
  let seg_len = 2 + exif_data_len in
  (* length field includes itself *)
  ...
  let pu16 buf off v =
    if big_endian then put_u16_be buf off v else put_u16_le buf off v
  in
  let pu32 buf off v =
    if big_endian then put_u32_be buf off v else put_u32_le buf off v
  in

  let tiff = 4 + 6 in
  (* offset of TIFF header inside app1 buffer *)
  (* Byte order mark *)
  if big_endian then Bytes.blit_string "MM" 0 app1 tiff 2
  else Bytes.blit_string "II" 0 app1 tiff 2;
  (* TIFF magic 0x002A *)
  ...
  (* Rational values stored after the IFD block. IFD block = 2 (count) + 3*12
     (entries) + 4 (next IFD) = 42 bytes Rationals start at tiff_start + 8 + 42
     = tiff_start + 50 i.e. offset-from-tiff-start = 50 for XRes, 58 for
     YRes. *)
  ...
  (* Entry 2: ResolutionUnit — tag 0x0128, type SHORT(3), count 1, value stored
     inline in the value_or_offset field. *)
  let e2 = e1 + 12 in
  pu16 app1 e2 0x0128;
  pu16 app1 (e2 + 2) 3;
  (* type = SHORT *)
  pu32 app1 (e2 + 4) 1;
  (* count *)
  pu16 app1 (e2 + 8) res_unit;

  (* value inline, upper 2 bytes stay 0 *)

  (* Next IFD offset = 0 (no more IFDs) — already zeroed *)

  (* Rational values *)
  let rv_base = tiff + xres_valoff in
  pu32 app1 rv_base dpi;
  (* XRes numerator *)
  pu32 app1 (rv_base + 4) 1;
  (* XRes denominator *)
  pu32 app1 (rv_base + 8) dpi;
  (* YRes numerator *)
  pu32 app1 (rv_base + 12) 1;
