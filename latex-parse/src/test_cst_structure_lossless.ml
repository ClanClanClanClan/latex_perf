(** Structure-lossless runtime gate (v26.3 §3 item C).
    ...
    deterministic, but a runtime check makes the property machine-verified on a
    curated subset.

    Curated subset: LP-Core fixtures under 1MB with no unclosed constructs. We
    use [corpora/roundtrip/] (15 synthetic edge cases shipped in v26.2) plus the
    v26.2.1 fix-integration fixtures. *)
...
let resolved_dirs = List.filter Sys.file_exists candidate_dirs
...
(** Drop fixtures that exceed 1 MB or contain unclosed constructs we
    pre-flagged. The roundtrip corpus has explicit "unclosed_*" files which
    v26.2 byte-tests but we exclude here. *)
...
let () =
  ...
  if n_fail = 0 then (
    ...
    Printf.eprintf "[cst-structure-lossless] FAIL: %d / %d fixtures diverged\n"
      n_fail total;
    List.iter
