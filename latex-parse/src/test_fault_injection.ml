...
let run_all_tests () =
  ...
  else printf "✗ Some tests failed - service needs hardening\n%!"
...
let interactive_mode () =
  ...
  Fault_injection.configure ~enabled:true ~probability:1.0 ~faults:[] ~seed:None;
...
let () =
  ...
  match args with
  | _ :: "--all" :: _ -> run_all_tests ()
  | _ :: "--test" :: test_name :: _ -> ignore (run_test test_name)
  | _ :: "--interactive" :: _ -> interactive_mode ()
  | _ ->
