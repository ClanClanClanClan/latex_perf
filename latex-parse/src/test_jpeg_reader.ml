(* ══════════════════════════════════════════════════════════════════════
   ...
   30+ test cases covering SOF0 dimensions, component counts, APP0 JFIF DPI
   (units 1 & 2), APP2 ICC profile detection and color space, APP14 Adobe
   transform values, missing markers, edge cases (non-JPEG, truncated, SOI+EOI
   only), and combined markers.
   ══════════════════════════════════════════════════════════════════════ *)
...
let () =
  ...
  run "SOF0: 1x1 minimum" (fun tag ->
      let opts =
        { Test_binary_gen.default_jpeg_opts with j_width = 1; j_height = 1 }
      in
  ...
  run "SOF0: wide 4000x100" (fun tag ->
      let opts =
        {
          Test_binary_gen.default_jpeg_opts with
          j_width = 4000;
          j_height = 100;
        }
      in
  ...
  run "SOF0: max u16 width 65535" (fun tag ->
      let opts =
        { Test_binary_gen.default_jpeg_opts with j_width = 65535; j_height = 1 }
      in
      with_jpeg opts (fun path ->
          match Jpeg_reader.read_jpeg_info path with
          | None -> expect false (tag ^ ": expected Some")
          | Some info -> expect (info.width = 65535) (tag ^ ": width=65535")));

  (* ════════════════════════════════════════════════════════════════════ 2.
     Component counts: 1 (gray), 3 (YCbCr), 4 (CMYK)
     ════════════════════════════════════════════════════════════════════ *)
  run "components: 1 (grayscale)" (fun tag ->
      ...
      with_jpeg opts (fun path ->
          match Jpeg_reader.read_jpeg_info path with
          | None -> expect false (tag ^ ": expected Some")
          | Some info -> expect (info.components = 1) (tag ^ ": components=1")));
  ...
  run "components: 3 (YCbCr)" (fun tag ->
      ...
      with_jpeg opts (fun path ->
          match Jpeg_reader.read_jpeg_info path with
          | None -> expect false (tag ^ ": expected Some")
          | Some info -> expect (info.components = 3) (tag ^ ": components=3")));
  ...
  run "components: 4 (CMYK)" (fun tag ->
      ...
      with_jpeg opts (fun path ->
          match Jpeg_reader.read_jpeg_info path with
          | None -> expect false (tag ^ ": expected Some")
          | Some info -> expect (info.components = 4) (tag ^ ": components=4")));

  (* ════════════════════════════════════════════════════════════════════ 3.
     APP0 JFIF DPI — units=1 (dots per inch)
     ════════════════════════════════════════════════════════════════════ *)
  ...
  (* ── APP0 JFIF DPI — units=2 (dots per cm) ───────────────────────── The
     generator only supports units=1, so we craft raw bytes for units=2 to test
     the reader's cm-to-inch conversion. *)
  run "JFIF DPI: units=2 (dots/cm) 118 -> ~300 DPI" (fun tag ->
      ...
      Bytes.set app0 11 '\002';
      (* units = dots per cm *)
      put_u16_be app0 12 118;
      (* X density *)
      put_u16_be app0 14 118;
      (* Y density *)
      ...
      with_jpeg_file data (fun path ->
          ...
          | Some info ->
              ...
              expect
                (abs_float (info.dpi_x -. expected) < 0.01)
                (Printf.sprintf "%s: dpi_x=%.2f expected ~%.2f" tag info.dpi_x
                   expected);
              expect
                (abs_float (info.dpi_y -. expected) < 0.01)
                (Printf.sprintf "%s: dpi_y=%.2f expected ~%.2f" tag info.dpi_y
                   expected)));

  (* ════════════════════════════════════════════════════════════════════ 4.
     APP2 ICC profile detection and color space
     ════════════════════════════════════════════════════════════════════ *)
  ...
  run "ICC multi-chunk: 2 chunks, color space from chunk 1" (fun tag ->
      ...
      with_jpeg opts (fun path ->
          ...
          | Some info ->
              expect info.has_icc_profile (tag ^ ": has_icc=true");
              expect
                (info.icc_color_space = `CMYK)
                (tag ^ ": icc=CMYK from chunk 1")));
  ...
  run "ICC multi-chunk: 3 chunks, color space from chunk 1" (fun tag ->
      ...
      with_jpeg opts (fun path ->
          ...
          | Some info ->
              expect info.has_icc_profile (tag ^ ": has_icc=true");
              expect
                (info.icc_color_space = `RGB)
                (tag ^ ": icc=RGB from chunk 1")));

  run "ICC multi-chunk: chunk 2 does not overwrite chunk 1 color space"
    (fun tag ->
      (* Use the generator for a 2-chunk CMYK profile, then verify the reader
         doesn't clobber icc_cs with Unknown from chunk 2. *)
      let opts =
        {
          Test_binary_gen.default_jpeg_opts with
          j_has_icc = true;
      ...
      with_jpeg opts (fun path ->
          ...
          | Some info ->
              expect info.has_icc_profile (tag ^ ": has_icc=true");
              expect
                (info.icc_color_space = `Gray)
                (tag ^ ": icc=Gray not overwritten by chunk 2")));

  (* ════════════════════════════════════════════════════════════════════ 5.
     APP14 Adobe marker with transform values 0, 1, 2
     ════════════════════════════════════════════════════════════════════ *)
  run "Adobe: transform=0 (CMYK)" (fun tag ->
      let opts =
        { Test_binary_gen.default_jpeg_opts with j_adobe_transform = 0 }
      in
      with_jpeg opts (fun path ->
          match Jpeg_reader.read_jpeg_info path with
          | None -> expect false (tag ^ ": expected Some")
  ...
  run "Adobe: transform=1 (YCbCr)" (fun tag ->
      let opts =
        { Test_binary_gen.default_jpeg_opts with j_adobe_transform = 1 }
      in
      with_jpeg opts (fun path ->
          match Jpeg_reader.read_jpeg_info path with
          | None -> expect false (tag ^ ": expected Some")
  ...
  run "Adobe: transform=2 (YCCK)" (fun tag ->
      let opts =
        { Test_binary_gen.default_jpeg_opts with j_adobe_transform = 2 }
      in
      with_jpeg opts (fun path ->
          match Jpeg_reader.read_jpeg_info path with
          | None -> expect false (tag ^ ": expected Some")
  ...
  (* ════════════════════════════════════════════════════════════════════ 6.
     Missing markers — defaults when absent
     ════════════════════════════════════════════════════════════════════ *)
  ...
  (* ════════════════════════════════════════════════════════════════════ 8.
     Combined: JPEG with all markers present
     ════════════════════════════════════════════════════════════════════ *)
  run "combined: all markers (JFIF + ICC + Adobe + SOF0)" (fun tag ->
      let opts =
        {
          Test_binary_gen.j_width = 640;
          j_height = 480;
          j_components = 3;
          j_dpi = 300;
          j_has_icc = true;
  ...
  (* ════════════════════════════════════════════════════════════════════ 9.
     EXIF APP1 DPI — big-endian and little-endian TIFF headers
     ════════════════════════════════════════════════════════════════════ *)
  ...
  run "EXIF DPI: JFIF(72) + EXIF(300) -> EXIF overrides" (fun tag ->
      ...
      with_jpeg_file data (fun path ->
          ...
          | Some info ->
              ...
              expect (info.dpi_x = 300.0)
                (Printf.sprintf
                   "%s: dpi_x=%.1f expected 300.0 (EXIF overrides JFIF)" tag
                   info.dpi_x);
              expect (info.dpi_y = 300.0)
                (Printf.sprintf
                   "%s: dpi_y=%.1f expected 300.0 (EXIF overrides JFIF)" tag
                   info.dpi_y)));
