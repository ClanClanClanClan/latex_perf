(** Direct unit tests for [find_math_ranges] and [is_in_math_range] (defined in
    ...
    individual producer tests. Edge cases — unclosed math at EOF, nested envs,
    escape sequences, all 11 supported math envs — are exercised directly here. *)
...
let range_cases : (string * string * (int * int) list) list =
  [
    ...
    (* Named envs *)
    ("equation env", "\\begin{equation}E=mc^2\\end{equation}.", [ (0, 36) ]);
    ("equation* env", "\\begin{equation*}x\\end{equation*}.", [ (0, 33) ]);
    ("align env", "\\begin{align}a&=b\\end{align}.", [ (0, 28) ]);
    ("align* env", "\\begin{align*}a&=b\\end{align*}.", [ (0, 30) ]);
    ("gather env", "\\begin{gather}a\\end{gather}.", [ (0, 27) ]);
    ("gather* env", "\\begin{gather*}a\\end{gather*}.", [ (0, 29) ]);
    ("multline env", "\\begin{multline}a\\end{multline}.", [ (0, 31) ]);
    ("multline* env", "\\begin{multline*}a\\end{multline*}.", [ (0, 33) ]);
    ("eqnarray env", "\\begin{eqnarray}a&=&b\\end{eqnarray}.", [ (0, 35) ]);
    ("math env", "\\begin{math}a+b\\end{math}.", [ (0, 25) ]);
    ("displaymath env", "\\begin{displaymath}a\\end{displaymath}.", [ (0, 37) ]);
    ...
    ("unclosed paren at EOF", "trailing \\(stuff", [ (9, 16) ]);
    ("unclosed env at EOF", "\\begin{equation}E", [ (0, 17) ]);
  ]

(** [is_in_math_range] consistency: for every byte position [p] in source [s],
    [is_in_math_range (find_math_ranges s) p] must be [true] iff [p] lies inside
    one of the returned half-open ranges. *)
  ...
  List.iter
    (fun (label, src) ->
      ...
      for p = 0 to String.length src - 1 do
        let direct = List.exists (fun (a, b) -> a <= p && p < b) ranges in
