...
let () =
  ...
  for _=1 to trials do
    ...
    try
      ...
      if String.trim s1 <> String.trim s2 then (eprintf "[parser2-prop] roundtrip FAIL: %S -> %S -> %S\n%!" s s1 s2; pass := false)
    with _ -> (eprintf "[parser2-prop] parse FAIL: %S\n%!" s; pass := false)
  done;
  if !pass then (printf "[parser2-prop] PASS %d trials\n%!" trials; exit 0) else exit 1
