(** Comprehensive unit tests for Pdf_reader — 40+ cases covering parse_pdf /
    read_pdf_info with programmatically generated PDFs from
...
let info_of opts =
  ...
  Fun.protect
    ...
    (fun () -> Pdf_reader.read_pdf_info path)
...
let () =
  ...
  run "parse_pdf returns Some with objects" (fun tag ->
      ...
      | Some doc ->
          expect (Hashtbl.length doc.objects > 0) (tag ^ ": no objects")
      | None -> expect false (tag ^ ": None"));
  ...
  run "figures_without_alt = 1" (fun tag ->
      let opts =
        { default with has_struct_tree = true; figures_without_alt = 1 }
      in
      match info_of opts with
      | Some i ->
          expect
            (i.figures_without_alt = 1)
            (tag ^ Printf.sprintf ": got %d" i.figures_without_alt)
      | None -> expect false (tag ^ ": None"));

  run "figures_without_alt = 3" (fun tag ->
      let opts =
        { default with has_struct_tree = true; figures_without_alt = 3 }
      in
      match info_of opts with
      | Some i ->
          expect
            (i.figures_without_alt = 3)
            (tag ^ Printf.sprintf ": got %d" i.figures_without_alt)
      | None -> expect false (tag ^ ": None"));
  ...
  run "links_without_contents = 4" (fun tag ->
      ...
      | Some i ->
          expect
            (i.links_without_contents = 4)
            (tag ^ Printf.sprintf ": got %d" i.links_without_contents)
      | None -> expect false (tag ^ ": None"));

  (* ══════════════════════════════════════════════════════════════════════ §7
     Fonts — embedded vs not embedded, subset detection
     ══════════════════════════════════════════════════════════════════════ *)
  ...
  run "one embedded non-subsetted font" (fun tag ->
      let opts = { default with fonts = [ ("Helvetica", true, false) ] } in
      match info_of opts with
      | Some i -> (
          let n = List.length i.fonts in
          expect (n = 1) (tag ^ Printf.sprintf ": expected 1 font, got %d" n);
          match i.fonts with
          | [ (name, emb, sub) ] ->
  ...
  run "one non-embedded font" (fun tag ->
      let opts = { default with fonts = [ ("TimesRoman", false, false) ] } in
      match info_of opts with
      | Some i -> (
          match i.fonts with
          | [ (_, emb, _) ] -> expect (not emb) (tag ^ ": expected not embedded")
          | _ -> expect false (tag ^ ": wrong font count"))
      | None -> expect false (tag ^ ": None"));

  run "subsetted font has ABCDEF+ prefix" (fun tag ->
      let opts = { default with fonts = [ ("CMR10", true, true) ] } in
      match info_of opts with
      | Some i -> (
          match i.fonts with
          | [ (name, emb, sub) ] ->
              expect
                (String.length name > 7 && name.[6] = '+')
          ...
          | _ ->
              expect false
                (tag
                ^ Printf.sprintf ": wrong font count %d" (List.length i.fonts)))
      | None -> expect false (tag ^ ": None"));
  ...
  run "multiple fonts" (fun tag ->
      ...
      | Some i ->
          expect
            (List.length i.fonts = 3)
            (tag
            ^ Printf.sprintf ": expected 3 fonts, got %d" (List.length i.fonts)
            )
      | None -> expect false (tag ^ ": None"));

  run "embedded font has FontFile2" (fun tag ->
      let opts = { default with fonts = [ ("DejaVuSans", true, false) ] } in
      match info_of opts with
      | Some i -> (
          match i.fonts with
  ...
  run "non-subsetted font name has no plus sign" (fun tag ->
      let opts = { default with fonts = [ ("Palatino", true, false) ] } in
      match info_of opts with
      | Some i -> (
          match i.fonts with
          | [ (name, _, sub) ] ->
  ...
  run "no spot colours by default" (fun tag ->
      ...
      | Some i ->
          ...
          expect
            (not i.has_spot_colour_vector)
            (tag ^ ": has_spot_colour_vector")
      | None -> expect false (tag ^ ": None"));

  run "Separation spot colour present" (fun tag ->
      let opts =
        { default with has_spot_colour = true; has_spot_colour_vector = false }
      in
      match info_of opts with
      | Some i ->
  ...
  run "DeviceN spot colour vector present" (fun tag ->
      let opts =
        { default with has_spot_colour = true; has_spot_colour_vector = true }
      in
      match info_of opts with
      | Some i ->
          expect i.has_spot_colour (tag ^ ": expected has_spot_colour");
  ...
  run "streams_all_compressed = true by default" (fun tag ->
      match info_of default with
      | Some i -> expect i.streams_all_compressed (tag ^ ": expected true")
      | None -> expect false (tag ^ ": None"));
  ...
  run "uncompressed stream detected" (fun tag ->
      ...
      match info_of opts with
      | Some i ->
          expect (not i.streams_all_compressed) (tag ^ ": expected false")
      | None -> expect false (tag ^ ": None"));
  ...
  (* ══════════════════════════════════════════════════════════════════════ §10
     Edge cases — non-PDF, truncated, empty, encrypted
     ══════════════════════════════════════════════════════════════════════ *)
  ...
  run "nonexistent file returns None" (fun tag ->
      expect (Pdf_reader.read_pdf_info "/tmp/nonexistent_pdf_42.pdf" = None) tag);

  run "parse_pdf on non-PDF returns None" (fun tag ->
      let data = Bytes.of_string "Hello World" in
      with_pdf data (fun path -> expect (Pdf_reader.parse_pdf path = None) tag));
  ...
  (* ══════════════════════════════════════════════════════════════════════ §11
     Combined configurations — tagged PDF with all features
     ══════════════════════════════════════════════════════════════════════ *)
      ...
      let opts =
        {
          ...
          links_without_contents = 3;
        }
      in
      match info_of opts with
      | Some i ->
          expect
            (i.figures_without_alt = 2)
            (tag ^ Printf.sprintf ": figs = %d" i.figures_without_alt);
          expect
            (i.links_without_contents = 3)
  ...
  run "full kitchen sink: all features" (fun tag ->
      let opts =
        {
          Test_binary_gen.has_struct_tree = true;
          has_mark_info = true;
          has_lang = Some "ja";
          figures_without_alt = 2;
  ...
  run "read_pdf_info roundtrips parse_pdf" (fun tag ->
      let opts = { default with has_lang = Some "es"; has_mark_info = true } in
      let data = Test_binary_gen.make_pdf opts in
      let path = Test_binary_gen.write_to_temp_file data ".pdf" in
      Fun.protect
        ~finally:(fun () -> Sys.remove path)
