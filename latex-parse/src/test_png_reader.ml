(** Comprehensive tests for the Png_reader module.
    ...
    Generates synthetic PNG files using Test_binary_gen, writes them to
    temp files, reads them back with Png_reader.read_png_info, and
    validates the parsed metadata.  35+ test cases covering IHDR fields,
    pHYs DPI, tRNS, iCCP, sRGB, PLTE, edge cases, and combined chunks. *)

open Latex_parse_lib
open Test_helpers

(** Write [data] to a temp file, run [f path], then remove the file.
    Cleanup runs even when [f] raises. *)
...
let require_some tag = function
  ...
  | None -> expect false (tag ^ ": expected Some, got None");
            ...
            { Png_reader.width = -1; height = -1; bit_depth = -1;
              color_type = -1; has_trns = false; phys_x = -1; phys_y = -1;
              phys_unit = -1; has_iccp = false; iccp_name = "";
              has_srgb = false; plte_entries = -1 }

let () =
  (* ════════════════════════════════════════════════════════════════════
     1. Basic IHDR parsing — width / height
     ════════════════════════════════════════════════════════════════════ *)
  ...
  run "IHDR: small 1x1" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with width = 1; height = 1 } in
  ...
  run "IHDR: large 4096x2160" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with width = 4096; height = 2160 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "IHDR: non-square 640x480" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with width = 640; height = 480 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  (* ════════════════════════════════════════════════════════════════════
     2. Color types (0, 2, 3, 4, 6)
     ════════════════════════════════════════════════════════════════════ *)
  run "IHDR: color_type 0 (grayscale)" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with color_type = 0; bit_depth = 8 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "IHDR: color_type 2 (truecolor)" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with color_type = 2; bit_depth = 8 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "IHDR: color_type 3 (indexed)" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   color_type = 3; bit_depth = 8; plte_size = 16 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "IHDR: color_type 4 (grayscale+alpha)" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with color_type = 4; bit_depth = 8 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "IHDR: color_type 6 (truecolor+alpha)" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with color_type = 6; bit_depth = 8 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  (* ════════════════════════════════════════════════════════════════════
     4. pHYs — DPI / resolution metadata
     ════════════════════════════════════════════════════════════════════ *)
  ...
  run "tRNS: present on indexed color" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   color_type = 3; plte_size = 4; has_trns = true } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
          expect info.has_trns (tag ^ ": has_trns = true")));
  ...
  (* ════════════════════════════════════════════════════════════════════
     6. iCCP — embedded ICC profile
     ════════════════════════════════════════════════════════════════════ *)
  run "iCCP: present with name" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   has_iccp = true; iccp_name = "sRGB IEC61966-2.1" } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
          expect info.has_iccp (tag ^ ": has_iccp = true");
          expect (info.iccp_name = "sRGB IEC61966-2.1")
            (tag ^ ": iccp_name = " ^ info.iccp_name)));

  run "iCCP: short name" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   has_iccp = true; iccp_name = "P3" } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
          expect info.has_iccp (tag ^ ": has_iccp = true");
  ...
  (* ════════════════════════════════════════════════════════════════════
     7. sRGB — standard RGB rendering intent
     ════════════════════════════════════════════════════════════════════ *)
  ...
  (* ════════════════════════════════════════════════════════════════════
     8. PLTE — palette entries
     ════════════════════════════════════════════════════════════════════ *)
  run "PLTE: 1 entry" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   color_type = 3; bit_depth = 8; plte_size = 1 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "PLTE: 128 entries" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   color_type = 3; bit_depth = 8; plte_size = 128 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "PLTE: 256 entries (max)" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   color_type = 3; bit_depth = 8; plte_size = 256 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  (* ════════════════════════════════════════════════════════════════════
     9. Edge cases — malformed or non-PNG input
     ════════════════════════════════════════════════════════════════════ *)
  ...
  run "edge: truncated mid-IHDR returns None" (fun tag ->
      (* PNG magic (8 bytes) + partial IHDR chunk header (8 bytes) but no data *)
      let data = Bytes.create 20 in
      Bytes.blit_string "\137PNG\r\n\026\n" 0 data 0 8;
      (* chunk length = 13 *)
      Bytes.set data 8 '\000'; Bytes.set data 9 '\000';
      Bytes.set data 10 '\000'; Bytes.set data 11 '\013';
      (* chunk type = IHDR *)
      Bytes.blit_string "IHDR" 0 data 12 4;
      (* Only 4 bytes of IHDR data instead of 13 *)
      Bytes.set data 16 '\000'; Bytes.set data 17 '\000';
      Bytes.set data 18 '\000'; Bytes.set data 19 '\001';
      with_temp_png data (fun path ->
          let result = Png_reader.read_png_info path in
          expect (result = None) (tag ^ ": expected None")));
  ...
  run "edge: nonexistent path returns None" (fun tag ->
      let result = Png_reader.read_png_info "/tmp/nonexistent_test_png_42.png" in
      expect (result = None) (tag ^ ": expected None"));

  (* ════════════════════════════════════════════════════════════════════
     10. Combined — multiple optional chunks simultaneously
     ════════════════════════════════════════════════════════════════════ *)
  run "combined: sRGB + tRNS + pHYs 150dpi" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   has_srgb = true; has_trns = true; dpi = 150 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "combined: iCCP + PLTE + tRNS (indexed)" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   color_type = 3; bit_depth = 8; plte_size = 64;
                   has_iccp = true; iccp_name = "AdobeRGB";
                   has_trns = true } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "combined: all optional chunks enabled" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   ...
                   color_type = 3; plte_size = 32;
                   dpi = 300; has_trns = true;
                   has_iccp = true; iccp_name = "DisplayP3";
                   has_srgb = true } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "combined: grayscale+alpha 16-bit, 600 DPI, no optional" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   ...
                   color_type = 4; bit_depth = 16; dpi = 600 } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
  ...
  run "combined: truecolor+alpha sRGB only" (fun tag ->
      let opts = { Test_binary_gen.default_png_opts with
                   color_type = 6; bit_depth = 8; has_srgb = true } in
      let png = Test_binary_gen.make_png opts in
      with_temp_png png (fun path ->
          let info = require_some tag (Png_reader.read_png_info path) in
          expect (info.color_type = 6) (tag ^ ": color_type");
