let () =
  ...
  (* Run a small test *)
  let test_input = Bytes.of_string "\\section{test} hello world $x=1$ % comment\n" in
  let module Arena = Latex_parse_lib.Arena in
  let arena = Arena.create ~cap:1000 in
  let buf = Arena.current arena in
  
  (* Tokenize using SIMD *)
  let (status, token_count, issues) = Latex_parse_lib.Real_processor.run test_input buf in
