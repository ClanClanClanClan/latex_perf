...
(* ENC-018: Non-breaking hyphen U+2011 present outside URLs. v27.0.31: fix
   ...
   collected offsets map back to source positions for fix-edit emission. The URL
   backscan logic is preserved (look back for `\url{` not preceded by `}`).
  ...
  let run s =
    ...
    while !i < n - 2 do
      ...
      then (
        ...
        (* Skip if inside `\url{...}` — backward scan for `\url{` not shadowed
           by an intervening `}`. *)
        let in_url = ref false in
        let j = ref (!i - 1) in
        while !j >= 0 && not !in_url do
          if !j + 4 < n && String.sub s !j 5 = "\\url{" then in_url := true;
          if s.[!j] = '}' then j := -1 (* stop searching *) else decr j
        done;
