...
module T = struct
  type agg = { mutable hits:int; mutable dur_ms:float; mutable last_count:int }
end
...
let record ~(id:string) ~(count:int) ~(dur_ms:float) =
  Mutex.lock mu;
  let a = match Hashtbl.find_opt tbl id with
    | Some x -> x
    | None -> let x = { T.hits=0; dur_ms=0.; last_count=0 } in Hashtbl.add tbl id x; x
  in
  a.T.hits <- a.T.hits + 1;
