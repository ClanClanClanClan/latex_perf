...
module V25_lexer_baseline = struct
  ...
  let tokenize content =
    ...
    let[@inline] peek () =
      if !pos < len then Some content.[!pos] else None
    in
...
(* ===== MAIN TEST ===== *)
let () =
  printf "🔬 CORRECT V25 TOKENIZATION TEST\n";
  printf "=================================\n";
  ...
  (* Token analysis *)
  printf "\n--- TOKEN SAMPLE ---\n";
  let print_token i tok =
    let t = tok.Lexer_v25.token in
    match t with
    | TChar (uc, cc) ->
        let c = Uchar.to_int uc in
        let ch = if c >= 32 && c <= 126 then 
          String.make 1 (Char.chr c) 
        else Printf.sprintf "\\x%02x" c in
        let cat = match cc with
          | Letter -> "L" | Other -> "O" | Space -> "S"
          | MathShift -> "$" | BeginGroup -> "{" | EndGroup -> "}"
          | _ -> "?" in
        printf "%3d: [%s] %s\n" i cat ch
    | TMacro name -> printf "%3d: MACRO %s\n" i name
    | TParam n -> printf "%3d: PARAM #%d\n" i n
    | TGroupOpen -> printf "%3d: {\n" i
    | TGroupClose -> printf "%3d: }\n" i
    | TEOF -> printf "%3d: EOF\n" i
  in
  let rec take n = function
    ...
    | h::t -> h :: take (n-1) t
  in
  List.iteri print_token (take 50 !track_a_tokens);
  ...
  printf "≤18.7ms: %s (%.2fms)\n"  
    (if track_a_median <= 18.7 then "✅" else "❌") track_a_median;
  
  printf "\nExpected ~154K tokens, got %d (%.1fx)\n"
    track_a_count (float track_a_count /. 154293.0);
  
  printf "\n🏁 CORRECT V25 TEST COMPLETE\n"