#!/usr/bin/env python3
"""
Script to add soundness proofs for MATH rules.
"""

import re

def create_soundness_proof(rule_num):
    """Generate a soundness proof for MATH rule."""
    rule_id = f"math_{rule_num}"
    return f"""(** ** MATH-{rule_num} Soundness Proof *)
...
def add_math_proofs():
    """Add soundness proofs for MATH-001 through MATH-005."""
    ...
    for rule_num in range(1, 6):
        ...
        rule_id = f"math_{rule_num_str}"
        
        # Check if proof already exists
        if f"Theorem {rule_id}_soundness" in content:
            print(f"Proof for MATH-{rule_num_str} already exists, skipping")
            continue
        ...
        if not match:
            print(f"Rule definition for MATH-{rule_num_str} not found")
            continue
        ...
        print(f"Added soundness proof for MATH-{rule_num_str}")
    
    # Write the updated content back
    with open(filename, 'w') as f:
        f.write(content)
    
    print("Done! All MATH soundness proofs added.")
