...
(* Worker status *)
type worker_status = Idle | Busy of proof_job | Failed of string | Terminated
...
type job_result =
  ...
  | Timeout of { job_id : string; worker_id : string; timeout_ms : int }
...
let process_heartbeats farm =
  ...
  List.iter
    (fun worker ->
      if current_time -. worker.last_heartbeat > timeout then (
...
let update_heartbeat farm worker_id =
  ...
  | Some worker -> worker.last_heartbeat <- Unix.gettimeofday ()
  | None -> Printf.printf "Unknown worker: %s\n" worker_id
...
let get_stats farm =
  ...
  let throughput =
    if uptime > 0. then float_of_int farm.total_jobs_completed /. uptime else 0.
  in
  let success_rate =
    if farm.total_jobs_completed + farm.total_jobs_failed > 0 then
...
let pp_stats stats =
  ...
  Printf.printf "Workers: %d total (%d idle, %d busy, %d failed)\n"
    stats.total_workers stats.idle_workers stats.busy_workers
