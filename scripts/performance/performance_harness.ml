...
let measure_once ?(measure_memory = false) ?(measure_gc = false) fn =
  ...
  let memory_mb =
    match start_memory with
    | Some start_mb ->
        let end_mb =
          (Gc.stat ()).heap_words * (Sys.word_size / 8) / 1024 / 1024
        in
...
let calculate_statistics measurements config =
  ...
  if len = 0 then
    ...
    let percentiles =
      List.map (fun p -> (p, percentile runtimes p)) config.target_percentiles
    in
...
let benchmark ?(config = default_config "unnamed") fn =
  ...
  let measurements =
    Array.make config.iterations
      { iteration = 0; runtime_ms = 0.0; memory_mb = None; gc_stats = None }
  in
  ...
  for i = 0 to config.iterations - 1 do
    if !total_time *. 1000. < float_of_int config.timeout_ms then (
      let _result, measurement =
        measure_once ~measure_memory:true ~measure_gc:true fn
      in
...
let pp_result result =
  ...
  printf "Completed: %d iterations in %.2fs\n"
    (Array.length result.measurements)
  ...
  printf "\nPercentiles:\n";
  List.iter
    (fun (p, v) -> printf "  P%.0f:      %.3f ms\n" p v)
    result.statistics.percentiles;

  (* Memory statistics if available *)
  let memory_measurements =
    Array.to_list result.measurements |> List.filter_map (fun m -> m.memory_mb)
  in
  if List.length memory_measurements > 0 then (
    let total_memory = List.fold_left ( +. ) 0.0 memory_measurements in
    let avg_memory =
      total_memory /. float_of_int (List.length memory_measurements)
    in
    printf "\nMemory Usage:\n";
...
let export_csv result filename =
  ...
  Array.iter
    (fun m ->
      let memory_str =
        match m.memory_mb with Some mb -> sprintf "%.3f" mb | None -> ""
      in
...
let compare_results baseline current =
  ...
  let improvement_pct =
    (baseline_median -. current_median) /. baseline_median *. 100.
  in
  ...
  else printf "Regression:  %.1f%% slower ⚠️\n" (abs_float improvement_pct);
  printf "\n"
  ...
  List.iter
    (fun (name, result) ->
      printf "  %-20s: %.3f ms median, %.1f ops/s\n" name
        result.statistics.median_ms result.throughput_per_second)
    results;
  printf "\n";
