...
let measure_variant_bytes tok = 
  (measure_variant_words tok) * word_size
...
let () =
  ...
  let results = [
    ...
    check_size "TEOF" 
      (measure_variant_bytes TEOF) 
      expected.teof max_drift;
  ] in
  ...
  if List.for_all (fun x -> x) results then begin
