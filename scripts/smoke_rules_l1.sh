...
# Smoke test L1 rules via /expand?layer=l1
# Supports optional forbid: [ID ...] and expect_msgs: ["ID:Message", ...]
# Usage: bash scripts/smoke_rules_l1.sh [REST_URL]

REST_URL=${1:-http://127.0.0.1:8080/expand?layer=l1}
GOLDEN=specs/rules/l1_golden.yaml

if ! command -v jq >/dev/null 2>&1; then
  echo "[l1-smoke] ERROR: jq is required" >&2
  exit 1
fi

if [[ ! -f "$GOLDEN" ]]; then
  echo "[l1-smoke] ERROR: missing golden file $GOLDEN" >&2
  exit 1
fi
...
while IFS='|' read -r file expect forbid emsgs; do
  [[ -z "$file" ]] && continue
  if [[ ! -f "$file" ]]; then
    echo "[l1-smoke] MISSING: $file"; fail=$((fail+1)); continue
  fi

  payload=$(jq -Rs --arg k latex '{($k): .}' < "$file")
  resp=$(curl -s -X POST "$REST_URL" -H 'Content-Type: application/json' -d "$payload" || true)
  if [[ -z "$resp" ]]; then
    echo "[l1-smoke] FAIL: empty response for $file"; fail=$((fail+1)); continue
  fi
  ...
  if [[ $ok -eq 1 ]]; then
    echo "[l1-smoke] PASS: $file -> expect:[$expect] forbid:[$forbid] messages:[$(echo "$emsgs" | tr '\n' ';')]"
    pass=$((pass+1))
  else
    echo "[l1-smoke] FAIL: $file expected:[$expect] forbid:[$forbid] messages:[$(echo "$emsgs" | tr '\n' ';')] got: $(echo "$ids" | tr '\n' ' ')" >&2
    fail=$((fail+1))
  fi
done < <(
  awk '
...
echo "[l1-smoke] Summary: pass=$pass fail=$fail"
[[ $fail -eq 0 ]]
