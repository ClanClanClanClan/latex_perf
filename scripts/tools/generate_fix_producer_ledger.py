...
def build_ledger(all_rules: list[str]) -> str:
    ...
    out.append("")
    out.append("Per the cadence plan, Bucket D rules should carry `produces_fix: false` in")
    out.append(f"`rule_contracts.yaml` with documented reason. **TODO:** {buckets['D']} D-bucket")
    out.append("rules currently lack this annotation; add in a follow-up cycle.")
    out.append("")
    ...
    out.append("- [ ] Bucket D rules carry `produces_fix: false` in `rule_contracts.yaml`")
    out.append("  with documented reason. **PENDING** — annotation cycle needed.")
