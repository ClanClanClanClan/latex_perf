(* L0 Lexer Track A - ULTRA-OPTIMIZED IMPLEMENTATION V3 FIXED *)
(* Target: 32ms -> 20ms (37.5% improvement) via hotpath optimizations *)
(* FIXES: Corrected macro parsing boundary detection and eliminated all subtle bugs *)
...
(* MAIN OPTIMIZATION: Ultra-hot tokenization loop - FIXED VERSION *)
let tokenize_ultra input =
  ...
  if len = 0 then [||] else (
    ...
    (* OPTIMIZATION O7: Unroll most common cases first - FIXED VERSION *)
    while !pos < len do
      ...
      (* ESCAPE - FIXED macro parsing logic *)
      | 0 -> 
          incr pos;
          if !pos < len then (
            ...
            if macro_len > 0 then (
              (* Multi-char macro - FIXED with bounds checking *)
              assert (macro_start + macro_len <= len);
              ...
              (* Single-char command - FIXED with explicit pos management *)
              ...
              UltraArena.emit_packed_token arena packed;
              incr pos  (* CRITICAL: Ensure pos advances *)
            )
          )
...
(* Utilities for working with packed tokens - FIXED VERSION *)
module PackedToken = struct
  ...
  (* FIXED: Robust token string representation *)
  let to_string token =
    try
      if is_macro token then (
        let name = get_macro_name token in
        Printf.sprintf "TMacro(\"%s\")" name
      ) else if is_char token then (
        let chr = get_char token in
        Printf.sprintf "TChar('%c')" chr
      ) else (
        let catcode = get_catcode token in
