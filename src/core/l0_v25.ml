...
(* Import the VERIFIED lexer - formally proven implementation! *)
module Proven_Lexer = Lexer_verified
...
let lex_chunk ?prev bytes =
  ...
  | Some (tokens, cached_state) ->
      (* Cache hit - update version and timing *)
      let new_state = {
        cached_state with
  ...
  | None ->
      ...
      let new_state = {
        ...
        position = {
          ...
          line = new_proven_state.position.line;
          column = new_proven_state.position.column;
        };
      } in
