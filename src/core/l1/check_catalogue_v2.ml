...
let body_ok j =
  match j with
  | `List ts -> List.for_all token_ok ts
  | _ -> false
...
let check_entry idx = function
  | `Assoc props as entry ->
      ...
      let err msg =
        Printf.sprintf "entry[%d]: %s" idx msg in
      begin match find "name" with
      | Some (`String name) ->
          if not (name_ok name) then Error (err "invalid macro name")
          else begin
            (* mode *)
            begin match find "mode" with
            | Some (`String ("math"|"text"|"any")) -> Ok ()
            | _ -> Error (err "mode must be math|text|any")
            end |> function Error e -> Error e | Ok () ->
            (* arity *)
            begin match find "arity" with
            | Some (`Int a) when 0 <= a && a <= 9 -> Ok ()
            | _ -> Error (err "arity must be 0..9")
            end |> function Error e -> Error e | Ok () ->
            (* safety *)
            begin match find "safety" with
            | Some (`String "pure") -> Ok ()
            | _ -> Error (err "safety must be \"pure\"")
            end |> function Error e -> Error e | Ok () ->
            (* side_effects *)
            begin match find "side_effects" with
            | Some (`List []) -> Ok ()
            | _ -> Error (err "side_effects must be []")
            end |> function Error e -> Error e | Ok () ->
            (* expansion/body *)
            begin match find "expansion" with
            | Some b when body_ok b -> Ok ()
            | _ -> Error (err "expansion invalid (must be token list)")
            end |> function Error e -> Error e | Ok () ->
            (* non_expandable *)
            begin match find "non_expandable" with
            | Some (`Bool true) -> Ok ()
            | _ -> Error (err "non_expandable must be true for L1 catalogue")
            end
          end
...
let check_file path =
  ...
  match json with
  | `Assoc props ->
      begin match List.assoc_opt "schema_version" props with
      | Some (`String "v25r2") -> ()
      | _ -> failwith "schema_version must be v25r2"
      end;
      begin match List.assoc_opt "macros" props with
      | Some (`List items) ->
          let rec loop i = function
            | [] -> Ok ()
            | x::xs -> (match check_entry i x with
                        | Ok () -> loop (i+1) xs
                        | Error e -> Error e)
          in loop 0 items
