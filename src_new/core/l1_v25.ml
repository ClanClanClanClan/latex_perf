...
(* Import expander - using stub for now *)
module Proven_Expander = Expander_stub
module Catalog = Expander_stub  (* Macros are in the same module for now *)

(* Cache key type from resolution *)
type l1_cache_key = {
  l0 : L0_v25.l0_cache_key;
  fuel_left : int;
  token_hash : int;  (* Hash of tokens being expanded *)
}
...
let expand_delta ~fuel ~env delta =
  ...
  (* Compute cache key - include hash of tokens *)
  let token_hash = 
